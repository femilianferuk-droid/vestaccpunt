...
def emoji(name: str) -> str:
    """Возвращает Unicode-эмодзи. <tg-emoji> теги НЕ используем:
    премиум-эмодзи работают только если бот владеет соответствующим
    стикерпаком — иначе Telegram отвечает ENTITY_TEXT_INVALID и падает
    всё сообщение. Если бот получит свои ID — поменяй одну строку тут."""
...
def create_button(
    ...
    """
    # Премиум icon_custom_emoji_id отключены по тем же причинам, что и
    # <tg-emoji> в emoji(): ID должны принадлежать боту, иначе Telegram
    # падает. Используем Unicode-эмодзи, которые уже есть в text.
    kwargs = {'text': text}
    if callback_data:
