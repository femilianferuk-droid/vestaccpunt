...
def emoji(name: str) -> str:
    """Возвращает Unicode-эмодзи. <tg-emoji> теги НЕ используем — они работают
    только если бот владеет соответствующим премиум-эмодзи, иначе Telegram
    отвечает ENTITY_TEXT_INVALID и сообщение падает целиком."""
    return EMOJI_CHARS.get(name, '📌')
...
def create_button(
    ...
    Args:
        text: Текст кнопки (если начинается с обычного эмодзи - он будет убран,
              чтобы не дублировать премиум-эмодзи из icon).
        callback_data: Данные для callback
        url: Ссылка
        style: Стиль кнопки (primary, success, danger, default)
        icon: Ключ иконки из PREMIUM_EMOJI_IDS
    """
    # Раньше тут обрезался ведущий эмодзи из text, потому что его заменяла
    # премиум-иконка. Сейчас премиум-эмодзи отключены (entity_text_invalid),
    # поэтому эмодзи в тексте кнопки сохраняем как есть.
    ...
    if style and style in ['primary', 'success', 'danger', 'default']:
        kwargs['style'] = style
    # icon_custom_emoji_id намеренно НЕ передаём: премиум-эмодзи должны
    # принадлежать боту, иначе Telegram рендерит пустую кнопку или падает
    # с ошибкой. Используем только Unicode-эмодзи из текста.
...
async def cmd_admin(message: Message):
    ...
    if message.from_user.id not in ADMIN_IDS:
        await safe_answer(
            message,
            f'{emoji("cross")} <b>Доступ запрещен</b>',
        )
        return
...
def review_rating_keyboard(purchase_id: int) -> InlineKeyboardMarkup:
    ...
    builder = InlineKeyboardBuilder()
    builder.row(
        create_button("⭐", callback_data=f"review_rate_{purchase_id}_1", style="danger", icon="star"),
        create_button("⭐⭐", callback_data=f"review_rate_{purchase_id}_2", style="danger", icon="star"),
        create_button("⭐⭐⭐", callback_data=f"review_rate_{purchase_id}_3", style="default", icon="star"),
        create_button("⭐⭐⭐⭐", callback_data=f"review_rate_{purchase_id}_4", style="success", icon="star"),
        create_button("⭐⭐⭐⭐⭐", callback_data=f"review_rate_{purchase_id}_5", style="success", icon="star"),
    )
...
async def _save_review(callback_or_msg, state: FSMContext, comment: str, from_message: bool = False):
    ...
    async with async_session() as session:
        ...
        if not purchase or not purchase.listing_id:
            text = f'{emoji("cross")} <b>Не удалось сохранить отзыв.</b>'
            if from_message:
                await callback_or_msg.message.answer(text, reply_markup=main_menu_keyboard())
            else:
                await callback_or_msg.message.answer(text, reply_markup=main_menu_keyboard())
            return
        ...
        if existing.scalar_one_or_none():
            text = f'{emoji("info")} <b>Вы уже оставляли отзыв на эту покупку.</b>'
            if from_message:
                await callback_or_msg.message.answer(text, reply_markup=main_menu_keyboard())
            else:
                await callback_or_msg.message.answer(text, reply_markup=main_menu_keyboard())
