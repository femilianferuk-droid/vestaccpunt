...
from sqlalchemy import (BigInteger, Boolean, Column, DateTime, Float, Integer,
                        String, Text, select, func, text as sa_text)
...
def emoji(name):
    return f'<tg-emoji emoji-id="{EMOJI[name]}"> </tg-emoji>'

# ===== КЛАВИАТУРЫ =====
def main_menu_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="Купить аккаунт", callback_data="buy_account", icon_custom_emoji_id=EMOJI['buy']),
        InlineKeyboardButton(text="Мои покупки", callback_data="my_purchases", icon_custom_emoji_id=EMOJI['box'])
    )
    builder.row(
        InlineKeyboardButton(text="Профиль", callback_data="profile", icon_custom_emoji_id=EMOJI['profile']),
        InlineKeyboardButton(text="Пополнить", callback_data="deposit_balance", icon_custom_emoji_id=EMOJI['wallet'])
    )
    return builder.as_markup()

def countries_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="США • 20₽", callback_data="country_USA", icon_custom_emoji_id=EMOJI['location']))
    builder.row(InlineKeyboardButton(text="Назад", callback_data="main_menu", icon_custom_emoji_id=EMOJI['back']))
    return builder.as_markup()

def account_found_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="КУПИТЬ", callback_data="show_payment_methods", icon_custom_emoji_id=EMOJI['buy']))
    builder.row(InlineKeyboardButton(text="Назад", callback_data="buy_account", icon_custom_emoji_id=EMOJI['back']))
    return builder.as_markup()

def payment_methods_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Баланс бота", callback_data="pay_balance", icon_custom_emoji_id=EMOJI['wallet']))
    builder.row(InlineKeyboardButton(text="ЮMoney", callback_data="pay_yoomoney", icon_custom_emoji_id=EMOJI['money']))
    builder.row(InlineKeyboardButton(text="Crypto Bot", callback_data="pay_crypto", icon_custom_emoji_id=EMOJI['crypto']))
    builder.row(InlineKeyboardButton(text="Telegram Stars", callback_data="pay_stars", icon_custom_emoji_id=EMOJI['star']))
    builder.row(InlineKeyboardButton(text="Назад", callback_data="buy_account", icon_custom_emoji_id=EMOJI['back']))
    return builder.as_markup()
...
def check_payment_keyboard(payment_id: str, method: str, is_deposit: bool = True):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Проверить оплату", callback_data=f"{prefix}_{method}_{payment_id}", icon_custom_emoji_id=EMOJI['loading']))
    builder.row(InlineKeyboardButton(text="Отмена", callback_data="main_menu", icon_custom_emoji_id=EMOJI['cross']))
    return builder.as_markup()

def get_code_keyboard(purchase_id: int):
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Получить код", callback_data=f"get_code_{purchase_id}", icon_custom_emoji_id=EMOJI['code']))
    builder.row(InlineKeyboardButton(text="К покупкам", callback_data="my_purchases", icon_custom_emoji_id=EMOJI['box']))
    return builder.as_markup()

def profile_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Пополнить баланс", callback_data="deposit_balance", icon_custom_emoji_id=EMOJI['wallet']))
    builder.row(InlineKeyboardButton(text="Мои покупки", callback_data="my_purchases", icon_custom_emoji_id=EMOJI['box']))
    builder.row(InlineKeyboardButton(text="В меню", callback_data="main_menu", icon_custom_emoji_id=EMOJI['home']))
    return builder.as_markup()

def deposit_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="ЮMoney", callback_data="deposit_yoomoney", icon_custom_emoji_id=EMOJI['money']))
    builder.row(InlineKeyboardButton(text="Crypto Bot", callback_data="deposit_crypto", icon_custom_emoji_id=EMOJI['crypto']))
    builder.row(InlineKeyboardButton(text="Назад", callback_data="profile", icon_custom_emoji_id=EMOJI['back']))
    return builder.as_markup()
...
def admin_keyboard():
    ...
    builder.row(InlineKeyboardButton(text="Добавить аккаунты", callback_data="admin_add_accounts", icon_custom_emoji_id=EMOJI['add']))
    builder.row(InlineKeyboardButton(text="Управление балансом", callback_data="admin_balance", icon_custom_emoji_id=EMOJI['edit']))
    builder.row(InlineKeyboardButton(text="В меню", callback_data="main_menu", icon_custom_emoji_id=EMOJI['home']))
    return builder.as_markup()
...
async def check_yoomoney_payment(payment_id: str) -> bool:
    """Проверяет платеж через API YooMoney используя CLIENT_ID"""
    if not CLIENT_ID:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, data=data, timeout=30) as response:
                if response.status == 200:
                    ...
                    logger.info(f"YooMoney check response: {result}")
                    ...
                    logger.error(f"YooMoney API error: {response.status} - {error_text}")
        return False
    except Exception as e:
...
async def callback_main_menu(callback: CallbackQuery):
    ...
    await callback.message.edit_text(f'{emoji("home")}<b>Главное меню</b>\n\n<i>Выберите действие:</i>', reply_markup=main_menu_keyboard())
...
async def callback_buy_account(callback: CallbackQuery):
    ...
    text += f'{emoji("check")}Аккаунты в наличии' if account else f'{emoji("cross")}Нет доступных аккаунтов'
...
async def callback_country(callback: CallbackQuery):
    ...
    if account:
        ...
        text = (
            ...
            f'{emoji("location")}Страна: <b>{country}</b>\n'
            f'{emoji("money")}Цена: <b>{account.price}₽</b>\n\n'
            '<i>Нажмите КУПИТЬ для продолжения</i>'
        )
        await callback.message.edit_text(text, reply_markup=account_found_keyboard())
    else:
        await callback.message.edit_text(f'{emoji("cross")}<b>Нет доступных аккаунтов</b>', reply_markup=countries_keyboard())
...
async def callback_show_payment_methods(callback: CallbackQuery):
    ...
    text = (
        f'{emoji("buy")}<b>Покупка аккаунта</b>\n\n'
        f'{emoji("money")}Сумма к оплате: <b>{price}₽</b>\n\n'
...
async def callback_pay_balance(callback: CallbackQuery):
    ...
    if not account_id:
        await callback.message.edit_text(f'{emoji("cross")}<b>Ошибка. Начните заново.</b>', reply_markup=main_menu_keyboard())
        return
    ...
    if user.balance >= price:
        async with async_session() as session:
            ...
            if account.is_sold:
                await callback.message.edit_text(f'{emoji("cross")}<b>Аккаунт уже продан.</b>', reply_markup=main_menu_keyboard())
                return
...
async def callback_pay_yoomoney(callback: CallbackQuery):
    ...
    text = (
        f'{emoji("money")}<b>Оплата через ЮMoney</b>\n\n'
        f'Сумма: <b>{price}₽</b>\n'
        f'Кошелек: <code>{YOOMONEY_WALLET}</code>\n\n'
        f'<a href="{payment_url}">💳 Нажмите для оплаты</a>\n\n'
        f'⚠️ <b>После оплаты нажмите кнопку проверки</b>\n'
...
async def callback_pay_crypto(callback: CallbackQuery):
    ...
    await callback.message.edit_text(f'{emoji("loading")}<b>Создаю счет...</b>')
    ...
    if invoice and invoice.get("ok"):
        ...
        async with async_session() as session:
            payment_result = await session.execute(select(Payment).where(Payment.payment_id == payment_id))
            payment = payment_result.scalar_one_or_none()
            if payment:
                payment.payment_id = str(invoice_id)
                await session.commit()
        ...
        await callback.message.edit_text(f'{emoji("cross")}<b>Ошибка создания счета</b>', reply_markup=payment_methods_keyboard())

@router.callback_query(F.data == "pay_stars")
async def callback_pay_stars(callback: CallbackQuery):
    await callback.answer()
    await callback.message.edit_text(f'{emoji("star")}<b>Оплата Telegram Stars</b>\n\nНапишите: <b>@v3estnikov</b>', reply_markup=payment_methods_keyboard())

# ===== ПРОВЕРКА ОПЛАТЫ ДЛЯ ПОКУПКИ =====
...
async def callback_check_purchase(callback: CallbackQuery):
    ...
    await callback.message.edit_text(f'{emoji("loading")}<b>Проверяю оплату...</b>', reply_markup=check_payment_keyboard(payment_id, method, is_deposit=False))
    ...
    if success:
        ...
        if account_id:
            ...
            await callback.message.edit_text(f'{emoji("cross")}<b>Данные заказа утеряны.</b>', reply_markup=main_menu_keyboard())
    else:
        await callback.message.edit_text(
            f'{emoji("loading")}<b>Оплата не найдена</b>\n\nПопробуйте через 1-2 минуты.',
...
async def callback_check_deposit(callback: CallbackQuery):
    ...
    await callback.message.edit_text(f'{emoji("loading")}<b>Проверяю пополнение...</b>', reply_markup=check_payment_keyboard(payment_id, method, is_deposit=True))
    ...
    if success:
        async with async_session() as session:
            ...
            if payment and payment.status != "completed":
                ...
                await callback.message.edit_text(f'{emoji("info")}<b>Платеж уже зачислен</b>', reply_markup=main_menu_keyboard())
            else:
                await callback.message.edit_text(f'{emoji("cross")}<b>Платеж не найден</b>', reply_markup=main_menu_keyboard())
    else:
        await callback.message.edit_text(
            f'{emoji("loading")}<b>Пополнение не найдено</b>\n\nПопробуйте через 1-2 минуты.',
...
async def callback_get_code(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        await callback.message.edit_text(f'{emoji("loading")}<b>Получаю код...</b>')
        ...
        if code:
            ...
            text = (
                f'{emoji("check")}<b>Код получен!</b>\n\n'
                f'{emoji("tag")}Номер: <code>{account.phone}</code>\n'
                f'{emoji("lock")}Код: <code>{code}</code>\n\n'
                '⚠️ <i>Сохраните код</i>'
            )
            ...
            await callback.message.edit_text(
                f'{emoji("cross")}<b>Не удалось получить код</b>\n\nПопробуйте позже или @v3estnikov',
        ...
        if purchases:
            ...
            for purchase in purchases:
                ...
                if not purchase.code_sent:
                    builder.row(InlineKeyboardButton(text=f"Получить код • {phone}", callback_data=f"get_code_{purchase.id}", icon_custom_emoji_id=EMOJI['code']))
            
            builder.row(InlineKeyboardButton(text="В меню", callback_data="main_menu", icon_custom_emoji_id=EMOJI['home']))
            
            await callback.message.edit_text(text, reply_markup=builder.as_markup())
        else:
            await callback.message.edit_text(f'{emoji("box")}<b>Мои покупки</b>\n\nУ вас пока нет покупок.', reply_markup=main_menu_keyboard())
...
async def callback_profile(callback: CallbackQuery):
    ...
    text = (
        ...
        '━━━━━━━━━━━━━━\n\n'
        '━━ 📊 СТАТИСТИКА ━━\n'
        f'{emoji("box")}Покупок: <b>{purchases_count}</b>\n'
        f'{emoji("money")}Потрачено: <b>{(user.total_spent or 0):.0f}₽</b>\n'
        f'{emoji("clock")}С нами с: {user.created_at.strftime("%d.%m.%Y")}\n'
        '━━━━━━━━━━━━━━'
    )
...
async def callback_deposit_balance(callback: CallbackQuery):
    ...
    text = (
        ...
        f'{emoji("money")}<b>ЮMoney</b> - перевод на кошелек\n'
        f'{emoji("crypto")}<b>Crypto Bot</b> - криптовалютой\n\n'
        '<i>Минимум: 10₽</i>'
    )
...
async def callback_deposit_yoomoney(callback: CallbackQuery):
    await callback.answer()
    await callback.message.answer(f'{emoji("money")}<b>Введите сумму (от 10₽)</b>\n\n<i>Отправьте число в чат</i>')
    ...
    if data == "admin_stats":
        async with async_session() as session:
            ...
            stats_text = (
                ...
                f'{emoji("profile")}Пользователей: <b>{total_users}</b>\n'
                f'{emoji("box")}Аккаунтов: <b>{total_accounts}</b>\n'
                f'{emoji("check")}Верифицировано: <b>{verified_accounts}</b>\n'
                f'{emoji("buy")}Продано: <b>{sold_accounts}</b>\n'
                f'{emoji("box")}Покупок: <b>{total_purchases}</b>\n'
...
async def handle_text(message: Message):
        ...
        try:
            ...
            if method == "yoomoney":
                ...
                await message.answer(
                    f'{emoji("money")}<b>Пополнение ЮMoney</b>\n\n'
                    f'Сумма: <b>{amount}₽</b>\n'
                    f'Кошелек: <code>{YOOMONEY_WALLET}</code>\n\n'
                    f'<a href="{payment_url}">💳 Нажмите для оплаты</a>\n\n'
                ...
                await message.answer(f'{emoji("loading")}Создаю счет...')
                ...
                if invoice and invoice.get("ok"):
                    ...
                    await message.answer(f'{emoji("cross")}<b>Ошибка создания счета</b>', reply_markup=deposit_keyboard())
    ...
    if hasattr(dp, 'awaiting_accounts') and user_id in dp.awaiting_accounts:
        ...
        if step == 'phone':
            ...
            await message.answer(f'{emoji("loading")}Отправляю код на {phone}...')
            ...
            if result['success']:
                ...
                await message.answer(f'{emoji("cross")}<b>Ошибка</b>\n{result.get("error")}', reply_markup=admin_keyboard())
            ...
            await message.answer(f'{emoji("loading")}Проверяю код...')
            ...
            if result['success']:
                ...
                await message.answer(
                    ...
                    f'{emoji("check")}Верифицирован\n'
                    f'{emoji("money")}Цена: 20₽\n\n'
                ...
                del dp.awaiting_accounts[user_id]
                await message.answer(f'{emoji("cross")}<b>{result.get("error")}</b>', reply_markup=admin_keyboard())
        
        elif step == 'password':
            password = text
            phone = acc_data['phone']
            
            await message.answer(f'{emoji("loading")}Проверяю пароль...')
            ...
            if result['success']:
                ...
                await message.answer(f'{emoji("cross")}<b>{result.get("error")}</b>\nПопробуйте еще раз:')
    ...
    if hasattr(dp, 'awaiting_balance') and user_id in dp.awaiting_balance:
        ...
        if step == 'user_id':
            try:
                ...
                if not target_user:
                    await message.answer(f'{emoji("cross")}<b>Пользователь не найден</b>', reply_markup=admin_keyboard())
                ...
                await message.answer(f'{emoji("cross")}<b>Введите корректный ID</b>')
            ...
            try:
                ...
                await message.answer(f'{emoji("cross")}<b>Введите корректную сумму</b>')
