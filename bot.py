...
# Цены по умолчанию (6 стран × 2 типа 2FA)
DEFAULT_PRICES = {
    "США": 20.0, "США (2FA)": 15.0,
    "Россия": 15.0, "Россия (2FA)": 10.0,
    "Индия": 10.0, "Индия (2FA)": 7.0,
    "Германия": 18.0, "Германия (2FA)": 13.0,
    "Бразилия": 8.0, "Бразилия (2FA)": 5.0,
    "Индонезия": 9.0, "Индонезия (2FA)": 6.0,
}
...
async def run_migrations():
    ...
    migrations = [
        ...
        "ALTER TABLE accounts ADD COLUMN IF NOT EXISTS has_2fa BOOLEAN DEFAULT FALSE",
...
EMOJI_CHARS = {
    ...
    'session': '📁', 'auto': '🤖', 'shield': '🛡️', 'phone': '📱',
    'search': '🔍', 'alert': '⚠️', 'country': '🌍', 'twofa': '🔒',
    'price': '💲',
}
...
async def countries_keyboard() -> InlineKeyboardMarkup:
    """Клавиатура выбора страны с актуальными ценами и фильтром 2FA"""
    builder = InlineKeyboardBuilder()
    prices = await get_all_prices()
    all_countries = ["США", "Россия", "Индия", "Германия", "Бразилия", "Индонезия"]
    
    for country in all_countries:
        price_no2fa = prices.get(country, DEFAULT_PRICES.get(country, 20))
        price_2fa = prices.get(f"{country} (2FA)", DEFAULT_PRICES.get(f"{country} (2FA)", price_no2fa * 0.7))
        flag = COUNTRY_FLAGS.get(country, "")
        
        builder.row(
            create_button(
                f"{flag} {country} • {price_no2fa:.0f}₽",
                callback_data=f"country_{country}_no2fa",
                style="primary",
                icon="location"
            )
        )
        builder.row(
            create_button(
                f"{flag} {country} (2FA) • {price_2fa:.0f}₽",
                callback_data=f"country_{country}_2fa",
                style="danger",
                icon="shield"
            )
        )
...
async def price_settings_keyboard() -> InlineKeyboardMarkup:
    ...
    for country in all_countries:
        ...
        price_2fa = prices.get(f"{country} (2FA)", DEFAULT_PRICES.get(f"{country} (2FA)", price * 0.7))
        flag = COUNTRY_FLAGS.get(country, "")
        ...
        builder.row(
            create_button(
                f"{flag} {country} (2FA): {price_2fa:.0f}₽",
                callback_data=f"set_price_{country}_2FA",
                style="danger",
                icon="shield"
            )
        )
...
def session_2fa_choice_keyboard() -> InlineKeyboardMarkup:
    """Клавиатура выбора 2FA для загруженной сессии"""
    builder = InlineKeyboardBuilder()
    builder.row(create_button("🔒 Есть 2FA", callback_data="session_2fa_yes", style="danger", icon="shield"))
    builder.row(create_button("🔓 Нет 2FA", callback_data="session_2fa_no", style="success", icon="check"))
    builder.row(create_button("Отмена", callback_data="admin", style="default", icon="cross"))
    return builder.as_markup()
...
async def check_low_accounts_and_notify():
    """
    Проверяет количество доступных аккаунтов по всем странам и типам.
    ...
    try:
        async with async_session() as session:
            ...
            result = await session.execute(
                select(Account.country, Account.has_2fa, func.count(Account.id))
                .where(Account.is_sold == False, Account.is_verified == True)
                .group_by(Account.country, Account.has_2fa)
            )
            stats = result.all()
            
            # Формируем словарь для быстрого поиска
            stats_dict = {}
            for country, has_2fa, count in stats:
                stats_dict[(country, has_2fa)] = count
            
            # Проверяем все возможные категории
            low_categories = []
            all_countries = ["США", "Россия", "Индия", "Германия", "Бразилия", "Индонезия"]
            
            for country in all_countries:
                for has_2fa in [False, True]:
                    count = stats_dict.get((country, has_2fa), 0)
                    
                    if count <= LOW_ACCOUNTS_THRESHOLD:
                        flag = COUNTRY_FLAGS.get(country, "")
                        twofa_text = "🔒 2FA" if has_2fa else "🔓 без 2FA"
                        low_categories.append(
                            f"{flag} <b>{country}</b> ({twofa_text}): <b>{count} шт.</b>"
                        )
            
            # Если есть категории с низким количеством - отправляем уведомление
            if low_categories:
                alert_text = (
                    f'{emoji("alert")} <b>⚠️ ВНИМАНИЕ! Заканчиваются аккаунты!</b>\n\n'
                    f'Следующие категории имеют ≤{LOW_ACCOUNTS_THRESHOLD} аккаунтов:\n\n'
                    + '\n'.join(low_categories) +
                ...
                logger.info(f"Low accounts alert: {len(low_categories)} categories have low stock")
...
async def get_available_account(country: str = None, has_2fa: bool = None) -> Optional[Account]:
    """Получает первый доступный для продажи аккаунт по фильтрам"""
    async with async_session() as session:
        ...
        if has_2fa is not None:
            query = query.where(Account.has_2fa == has_2fa)
...
async def get_available_countries() -> list:
    """Возвращает список всех доступных категорий с количеством"""
    async with async_session() as session:
        result = await session.execute(
            select(Account.country, Account.has_2fa, func.count(Account.id))
            .where(Account.is_sold == False, Account.is_verified == True)
...
async def cb_buy_account(callback: CallbackQuery):
    """Покупка аккаунта - выбор страны и типа 2FA"""
    await callback.answer()
    ...
    text = f'{emoji("location")} <b>Выберите страну и тип аккаунта</b>\n\n'
    
    if available:
        # Группируем по странам для красивого отображения
        countries_with_stock = set()
        for c, h2fa, count in available:
            if count > 0:
                countries_with_stock.add(c)
        
        text += f'{emoji("check")} <b>Доступны аккаунты из {len(countries_with_stock)} стран</b>\n'
        text += f'{emoji("shield")} <b>2FA аккаунты дешевле!</b>\n\n'
        text += '<i>🔓 Без 2FA — обычная цена</i>\n'
        text += '<i>🔒 С 2FA — со скидкой ~30%</i>'
    else:
...
async def cb_country(callback: CallbackQuery):
    """Выбор страны и типа 2FA - поиск аккаунта"""
    await callback.answer()
    
    if not await require_subscription(callback):
        return
    
    # Парсим callback_data: country_США_no2fa или country_США_2fa
    parts = callback.data.replace("country_", "").split("_")
    country = parts[0]
    has_2fa = parts[1] == "2fa" if len(parts) > 1 else False
    
    account = await get_available_account(country, has_2fa)
    
    if account:
        # Определяем ключ цены
        price_key = f"{country} (2FA)" if has_2fa else country
        price = await get_country_price(price_key)
        ...
        flag = COUNTRY_FLAGS.get(country, "")
        twofa_emoji = "🔒" if has_2fa else "🔓"
        twofa_text = "С 2FA (облачная защита)" if has_2fa else "Без 2FA"
        twofa_desc = (
            "Требуется пароль облачной защиты Telegram"
            if has_2fa
            else "Стандартный аккаунт без дополнительной защиты"
        )
        ...
        text = (
            ...
            f'{emoji("shield")} <b>Тип защиты:</b> {twofa_emoji} {twofa_text}\n'
            f'{emoji("info")} <b>Особенность:</b>\n<i>{twofa_desc}</i>\n'
            f'{emoji("tag")} <b>Номер телефона:</b>\n<code>{account.phone}</code>\n'
        ...
        no2fa_available = await get_available_account(country, False)
        fa_available = await get_available_account(country, True)
        
        flag = COUNTRY_FLAGS.get(country, "")
        text = (
            f'{emoji("cross")} <b>❌ Нет доступных аккаунтов</b>\n\n'
            f'{emoji("location")} Страна: {flag} <b>{country}</b>\n'
            f'{emoji("shield")} Тип: {"🔒 с 2FA" if has_2fa else "🔓 без 2FA"}\n\n'
        )
        
        if no2fa_available and has_2fa:
            text += f'{emoji("info")} 💡 <b>Есть аккаунты без 2FA</b> — попробуйте выбрать их'
        elif fa_available and not has_2fa:
            text += f'{emoji("info")} 💡 <b>Есть аккаунты с 2FA</b> — они дешевле!'
        else:
            text += f'{emoji("info")} Попробуйте выбрать другую страну или подождите поступления новых аккаунтов'
        
        await callback.message.answer(text, reply_markup=await countries_keyboard())
...
async def cb_show_payment_methods(callback: CallbackQuery):
    ...
    has_2fa = pending.get('has_2fa', False)
    ...
    text = (
        ...
        f'{emoji("location")} Страна: {flag} <b>{country}</b>\n'
        f'{emoji("shield")} Тип: <b>{twofa_text}</b>\n'
    ...
    async with async_session() as session:
        ...
        if purchases:
            ...
            for purchase in purchases:
                ...
                has_2fa = account.has_2fa if account else False
                flag = COUNTRY_FLAGS.get(country, "")
                date = purchase.created_at.strftime('%d.%m.%y %H:%M')
                twofa_icon = "🔒" if has_2fa else "🔓"
                
                text += (
                    f'{twofa_icon} <code>{phone}</code>\n'
...
async def cb_admin(callback: CallbackQuery, state: FSMContext):
    ...
    if data == "admin_stats":
        async with async_session() as session:
            ...
            text = (
                ...
                f'━━━━━━━━━━━━━━━━━━\n\n'
                f'{emoji("location")} <b>Доступные аккаунты по категориям:</b>\n'
            )
            
            if country_stats:
                for country, has_2fa, count in country_stats:
                    flag = COUNTRY_FLAGS.get(country, "")
                    twofa_text = "🔒2FA" if has_2fa else "🔓"
                    text += f'{flag} {country} {twofa_text}: <b>{count} шт.</b>\n'
            else:
    ...
    # --- Аккаунты ---
        ...
        async with async_session() as session:
            ...
            for account in accounts:
                ...
                twofa = "🔒2FA" if account.has_2fa else "🔓"
    ...
    # --- Добавление аккаунта через .session файл ---
        ...
        await callback.message.answer(
            ...
            f'{emoji("info")} <i>После проверки нужно будет выбрать тип 2FA</i>',
            reply_markup=builder.as_markup()
        )
    ...
    # --- Цены ---
        ...
        await callback.message.answer(
            ...
            'Выберите страну и тип для изменения цены:\n\n'
            f'{emoji("info")} <i>Цены можно настроить отдельно для аккаунтов с 2FA и без</i>',
...
# ===== ОБРАБОТЧИК 2FA ВЫБОРА ДЛЯ SESSION ФАЙЛА =====

@router.callback_query(F.data.startswith("session_2fa_"))
async def cb_session_2fa_choice(callback: CallbackQuery):
    """Обработчик выбора 2FA для загруженной сессии"""
    await callback.answer()
    
    if callback.from_user.id not in ADMIN_IDS:
        return
    
    choice = callback.data.replace("session_2fa_", "")
    has_2fa = choice == "yes"
    
    session_data = pending_session_data.get(callback.from_user.id)
    if not session_data:
        await callback.message.answer(
            f'{emoji("cross")} <b>Данные сессии утеряны. Загрузите файл заново.</b>',
            reply_markup=admin_keyboard()
        )
        return
    ...
    # Определяем цену в зависимости от 2FA
    price_key = f"{country} (2FA)" if has_2fa else country
    price = await get_country_price(price_key)
    ...
    async with async_session() as session:
        ...
        if existing_account:
            ...
            existing_account.has_2fa = has_2fa
    ...
    pending_session_data.pop(callback.from_user.id, None)
    
    flag = COUNTRY_FLAGS.get(country, "")
    twofa_text = "🔒 С 2FA" if has_2fa else "🔓 Без 2FA"
...
async def h_session_file(message: Message, state: FSMContext):
    ...
    if result['success']:
        # Сохраняем данные сессии и запрашиваем выбор 2FA
        pending_session_data[message.from_user.id] = {
            'phone': result['phone'],
        ...
        await message.answer(
            f'{emoji("check")} <b>✅ Сессия успешно проверена!</b>\n\n'
            f'{emoji("tag")} Номер телефона: <code>{result["phone"]}</code>\n'
            f'{emoji("location")} Страна: {COUNTRY_FLAGS.get(result["country"], "")} <b>{result["country"]}</b>\n\n'
            f'{emoji("shield")} <b>Выберите тип аккаунта:</b>\n\n'
            '🔒 <b>С 2FA</b> — если на аккаунте включена двухфакторная аутентификация\n'
            '🔓 <b>Без 2FA</b> — если двухфакторная аутентификация не используется',
            reply_markup=session_2fa_choice_keyboard()
        )
    else:
        await message.answer(
...
async def h_text(message: Message):
    ...
    if hasattr(dp, 'awaiting_accounts') and user_id in dp.awaiting_accounts:
        ...
        # Шаг 2: Код подтверждения
            ...
            if result['success']:
                # Сохраняем во временное хранилище для выбора 2FA
                pending_session_data[user_id] = {
                    'phone': phone,
                    'country': country,
                    'session_string': result['session_string'],
                    'session_json': result['session_json']
                ...
                await status_msg.edit_text(
                    f'{emoji("check")} <b>✅ Код верный! Аккаунт верифицирован!</b>\n\n'
                    f'{emoji("tag")} Номер: <code>{phone}</code>\n'
                    f'{emoji("location")} Страна: {flag} <b>{country}</b>\n\n'
                    f'{emoji("shield")} <b>Выберите тип аккаунта:</b>\n\n'
                    '🔒 <b>С 2FA</b> — двухфакторная аутентификация\n'
                    '🔓 <b>Без 2FA</b> — без дополнительной защиты',
                    reply_markup=session_2fa_choice_keyboard()
                )
                ...
                await status_msg.edit_text(
                    ...
                    '<i>Пароль был создан при настройке 2FA в Telegram</i>'
                )
        ...
        # Шаг 3: 2FA пароль
            ...
            if result['success']:
                ...
                await status_msg.edit_text(
                    f'{emoji("check")} <b>✅ Пароль верный! 2FA пройдена!</b>\n\n'
                    f'{emoji("tag")} Номер: <code>{phone}</code>\n'
                    f'{emoji("location")} Страна: {flag} <b>{country}</b>\n\n'
                    f'{emoji("shield")} <b>Выберите тип аккаунта:</b>\n\n'
                    '🔒 <b>С 2FA</b> — двухфакторная аутентификация\n'
...
async def main():
    ...
    logger.info(f"Countries: {list(COUNTRY_CODES.values())}")
