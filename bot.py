...
async def check_yoomoney_payment(payment_id: str) -> dict:
    """Проверяет платеж и возвращает результат с сообщением"""
    if not CLIENT_ID:
        return {'success': False, 'message': 'CLIENT_ID не настроен'}
    ...
    try:
        ...
        data = {"label": payment_id, "records": 30, "type": "deposition"}
        
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, data=data, timeout=30) as response:
                if response.status == 200:
                    result = await response.json()
                    logger.info(f"YooMoney API response: {result}")
                    
                    if "error" in result:
                        return {'success': False, 'message': result.get('error', 'Ошибка API')}
                    
                    operations = result.get("operations", [])
                    if not operations:
                        return {'success': False, 'message': 'Платеж не найден. Проверьте, выполнили ли вы перевод.'}
                    ...
                    for op in operations:
                        if op.get("label") == payment_id:
                            ...
                            if status == "success":
                                ...
                                return {'success': False, 'message': f'Статус платежа: {status}. Ожидайте подтверждения.'}
                    
                    return {'success': False, 'message': 'Платеж с таким ID не найден в истории операций'}
                else:
                    error_text = await response.text()
                    logger.error(f"YooMoney API error: {response.status} - {error_text}")
                    return {'success': False, 'message': f'Ошибка API: {response.status}'}
                    
    except asyncio.TimeoutError:
        return {'success': False, 'message': 'Превышено время ожидания ответа от YooMoney'}
    except Exception as e:
...
async def callback_check_purchase(callback: CallbackQuery):
    ...
    if method == "yoomoney":
        ...
        logger.info(f"Purchase check result: {result}")
...
async def callback_check_deposit(callback: CallbackQuery):
    ...
    if method == "yoomoney":
        result = await check_yoomoney_payment(payment_id)
        logger.info(f"Deposit check result: {result}")
...
async def callback_admin(callback: CallbackQuery, state: FSMContext):
    ...
    if data == "admin_stats":
        ...
        section = data.replace("set_media_", "")
...
# ===== ОБРАБОТЧИК МЕДИА ДЛЯ АДМИНА =====
@router.message(StateFilter(MediaStates.waiting_for_media), F.content_type.in_([ContentType.PHOTO, ContentType.VIDEO, ContentType.ANIMATION]))
