...
async def process_purchase(telegram_id: int, account_id: int, price: float, method: str) -> Optional[Purchase]:
    ...
    async with async_session() as session:
        # Ищем аккаунт через select, а не session.get
        result = await session.execute(
...
async def cb_pay_balance(callback: CallbackQuery):
    ...
    # Ищем пользователя по telegram_id
    result = await async_session().__aenter__()
    # Нужно правильно получить пользователя
    async with async_session() as session:
        result = await session.execute(
    ...
    if user and user.balance >= price:
        ...
        text = (
            ...
            f'{emoji("wallet")} Ваш баланс: <b>{user.balance if user else 0:.0f}₽</b>\n'
