...
def media_menu_keyboard():
    ...
    for name, cb in [("Главное меню","main_menu"),("Покупка","buy_account"),("Оплата","payment_methods"),("Профиль","profile"),("Покупки","my_purchases"),("Пополнение","deposit")]:
        builder.row(InlineKeyboardButton(text=name, callback_data=f"set_media_{cb}", icon_custom_emoji_id=PREMIUM_EMOJI_IDS.get(cb.split('_')[0], PREMIUM_EMOJI_IDS['home'])))
...
async def verify_2fa_password(phone: str, password: str) -> dict:
    try:
        ...
        session_string = auth_data['client'].session.save(); await auth_data['client'].disconnect()
        pending_auth.pop(phone, None)
        return {'success': True, 'session_string': session_string}
...
async def get_available_account():
    async with async_session() as session:
        r = await session.execute(select(Account).where(Account.is_sold == False, Account.is_verified == True, Account.session_string != None, Account.session_string != "").limit(1))
        return r.scalar_one_or_none()
...
async def cmd_admin(message: Message):
    if message.from_user.id not in ADMIN_IDS: await message.answer(f'{pe("cross")} <b>Доступ запрещен</b>'); return
...
async def cb_buy_account(callback: CallbackQuery):
    ...
    text = f'{pe("location")} <b>Выберите страну</b>\n\n{pe("check") if account else pe("cross")} {"Аккаунты в наличии" if account else "Нет доступных"}'
...
async def cb_show_payment(callback: CallbackQuery):
    ...
    text = f'{pe("buy")} <b>Покупка аккаунта</b>\n\n{pe("money")} Сумма: <b>{pending.get("price", 20)}₽</b>\n\n<i>Выберите способ:</i>'
...
async def cb_pay_balance(callback: CallbackQuery):
    ...
    if not account_id: await callback.message.answer(f'{pe("cross")} <b>Ошибка</b>', reply_markup=main_menu_keyboard()); return
    if user.balance >= price:
        async with async_session() as session:
            user = await session.get(User, user.id); account = await session.get(Account, account_id)
            if account.is_sold: await callback.message.answer(f'{pe("cross")} <b>Продан</b>', reply_markup=main_menu_keyboard()); return
    ...
    if inv and inv.get("status") == "paid":
        ...
        if account_id:
            async with async_session() as session:
                ...
                if account.is_sold: await callback.message.answer(f'{pe("cross")} <b>Продан</b>', reply_markup=main_menu_keyboard()); return
                pr = await session.execute(select(Payment).where(Payment.payment_id == payment_id)); pr = pr.scalar_one_or_none()
                ...
                session.add(purchase); await session.commit(); await session.refresh(purchase)
                await callback.message.answer(f'{pe("check")} <b>Оплата подтверждена!</b>\n\n{pe("tag")} Номер: <code>{account.phone}</code>\n{pe("money")} Сумма: <b>{price}₽</b>\n\nНажмите для кода:', reply_markup=get_code_keyboard(purchase.id))
...
async def cb_check_deposit(callback: CallbackQuery):
    ...
    if inv and inv.get("status") == "paid":
        async with async_session() as session:
            pr = await session.execute(select(Payment).where(Payment.payment_id == payment_id)); payment = pr.scalar_one_or_none()
            if payment and payment.status != "completed":
                payment.status = "completed"
                user = await session.get(User, callback.from_user.id); deposit_amount = payment.amount; user.balance += deposit_amount
                await session.commit()
                text = f'{pe("check")} <b>Баланс пополнен!</b>\n\n{pe("money")} Зачислено: <b>{deposit_amount:.2f}₽</b>\n{pe("wallet")} Баланс: <b>{user.balance:.2f}₽</b>'
                builder = InlineKeyboardBuilder(); builder.row(InlineKeyboardButton(text="В меню", callback_data="main_menu", icon_custom_emoji_id=PREMIUM_EMOJI_IDS['home']))
                await callback.message.answer(text, reply_markup=builder.as_markup())
    else: await callback.answer("⏳ Не найдено", show_alert=True)
...
async def cb_get_code(callback: CallbackQuery):
    ...
    async with async_session() as session:
        r = await session.execute(select(Purchase).where(Purchase.id == purchase_id)); purchase = r.scalar_one_or_none()
        if not purchase or purchase.user_id != callback.from_user.id: await callback.answer("Не найдена", show_alert=True); return
        if purchase.code_sent: await callback.answer("Уже получен", show_alert=True); return
        account = await session.get(Account, purchase.account_id)
        if not account or not account.session_string: await callback.answer("Аккаунт не найден", show_alert=True); return
        await callback.message.answer(f'{pe("loading")} <b>Получаю код...</b>')
        code = await get_code_from_session(account.session_string)
        if code:
            purchase.code_sent = True; await session.commit()
            builder = InlineKeyboardBuilder(); builder.row(InlineKeyboardButton(text="В меню", callback_data="main_menu", icon_custom_emoji_id=PREMIUM_EMOJI_IDS['home']))
            await callback.message.answer(f'{pe("check")} <b>Код получен!</b>\n\n{pe("tag")} Номер: <code>{account.phone}</code>\n{pe("lock")} Код: <code>{code}</code>\n\n⚠️ <i>Сохраните</i>', reply_markup=builder.as_markup())
        else: await callback.message.answer(f'{pe("cross")} <b>Не удалось</b>\n\n@v3estnikov', reply_markup=get_code_keyboard(purchase_id))
    ...
    async with async_session() as session:
        ...
        if purchases:
            ...
            for p in purchases:
                ...
                text += f'{"✅" if p.code_sent else "⏳"} <code>{account.phone if account else "Н/Д"}</code> • {p.amount}₽ • {p.created_at.strftime("%d.%m.%y")}\n'
                if not p.code_sent: builder.row(InlineKeyboardButton(text=f"Получить код • {account.phone if account else 'Н/Д'}", callback_data=f"get_code_{p.id}", icon_custom_emoji_id=PREMIUM_EMOJI_IDS['code']))
            builder.row(InlineKeyboardButton(text="В меню", callback_data="main_menu", icon_custom_emoji_id=PREMIUM_EMOJI_IDS['home']))
            await send_media_message(callback, "my_purchases", text, builder.as_markup())
        else: await send_media_message(callback, "my_purchases", f'{pe("box")} <b>Мои покупки</b>\n\nУ вас пока нет покупок.', main_menu_keyboard())
...
async def cb_profile(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        cnt = r.scalar() or 0
    text = f'{pe("profile")} <b>Профиль</b>\n\n{pe("tag")} ID: <code>{user.telegram_id}</code>\n{pe("profile")} @{user.username or "нет"}\n\n━━━ 💰 БАЛАНС ━━━\n{pe("wallet")} <b>{user.balance:.0f}₽</b>\n━━━━━━━━━━━━━━\n\n━━ 📊 СТАТИСТИКА ━━\n{pe("box")} Покупок: <b>{cnt}</b>\n{pe("money")} Потрачено: <b>{(user.total_spent or 0):.0f}₽</b>\n{pe("clock")} С нами: {user.created_at.strftime("%d.%m.%Y")}\n━━━━━━━━━━━━━━'
...
async def cb_sbp_paid(callback: CallbackQuery, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder(); builder.row(InlineKeyboardButton(text="Отмена", callback_data="main_menu", icon_custom_emoji_id=PREMIUM_EMOJI_IDS['cross']))
...
async def h_sbp_screenshot(message: Message, state: FSMContext):
    ...
    await message.answer(f'{pe("check")} <b>Скриншот отправлен на проверку!</b>', reply_markup=main_menu_keyboard())
    async with async_session() as session:
        pr = await session.execute(select(Payment).where(Payment.payment_id == payment_id)); payment = pr.scalar_one_or_none()
        if payment:
...
async def cb_sbp_approve(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        if payment and payment.status != "completed":
            payment.status = "completed"
            user = await session.execute(select(User).where(User.telegram_id == user_id)); user = user.scalar_one_or_none()
            if user:
                old_balance = user.balance; user.balance += payment.amount; await session.commit()
                # Админу
                await callback.message.edit_caption(f'{callback.message.caption}\n\n{pe("check")} <b>ОДОБРЕНО</b>\n💰 Баланс был: {old_balance:.0f}₽ → стал: {user.balance:.0f}₽', reply_markup=None)
                # Пользователю
                try: await bot.send_message(user_id, f'{pe("check")} <b>Платеж одобрен!</b>\n\n{pe("money")} Зачислено: <b>{payment.amount}₽</b>\n{pe("wallet")} Баланс: <b>{user.balance:.0f}₽</b>')
                except: pass
...
async def cb_admin_sbp(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        if payments:
            await callback.message.answer(f'{pe("sbp")} <b>Загружаю платежи...</b>')
            for payment in payments:
                user = await get_user(payment.user_id)
                try:
                    await bot.send_photo(callback.from_user.id, payment.screenshot_file_id,
                        caption=f'{pe("sbp")} <b>СБП платеж</b>\n\n{pe("profile")} ID: <code>{payment.user_id}</code>\n@{user.username or "нет"}\n{pe("money")} Сумма: <b>{payment.amount}₽</b>\n{pe("wallet")} Баланс: <b>{user.balance:.0f}₽</b>\n{pe("info")} ID: <code>{payment.payment_id}</code>',
                        reply_markup=sbp_approve_keyboard(payment.payment_id, payment.user_id))
                except: pass
            await callback.message.answer(f'{pe("info")} <b>Проверьте платежи выше</b>', reply_markup=admin_keyboard())
        else: await callback.message.answer(f'{pe("info")} <b>Нет платежей</b>', reply_markup=admin_keyboard())
...
async def cb_admin(callback: CallbackQuery, state: FSMContext):
    ...
    if data == "admin_stats":
        ...
        await callback.message.answer(f'{pe("media")} <b>Управление медиа</b>\n\nВыберите раздел:', reply_markup=media_menu_keyboard())
...
async def h_text(message: Message, state: FSMContext):
            ...
            if method == "sbp":
                ...
                if invoice and invoice.get("ok"):
                    ...
                    async with async_session() as session:
                        ...
                        if p: p.payment_id = str(r.get("invoice_id")); await session.commit()
                    await message.answer(f'{pe("crypto")} <b>Пополнение Crypto Bot</b>\n\nСумма: <b>{amount}₽</b>\n\n<a href="{r.get("pay_url")}">💳 Нажмите для оплаты</a>\n\n⚠️ После оплаты нажмите проверку', reply_markup=deposit_crypto_check_keyboard(str(r.get("invoice_id"))), disable_web_page_preview=True)
    ...
    if hasattr(dp, 'awaiting_accounts') and user_id in dp.awaiting_accounts:
        ...
        if step == 'phone':
            acc_data['phone'] = text
            r = await send_code_to_phone(text)
            if r['success']: acc_data['phone_code_hash'] = r['phone_code_hash']; acc_data['step'] = 'code'; await message.answer(f'{pe("check")} <b>Код отправлен</b>\n\nВведите код:')
            else: del dp.awaiting_accounts[user_id]; await message.answer(f'{pe("cross")} <b>Ошибка</b>', reply_markup=admin_keyboard())
        elif step == 'code':
            r = await verify_code_and_get_session(acc_data['phone'], text, acc_data['phone_code_hash'])
            if r['success']:
                async with async_session() as session:
                    ex = await session.execute(select(Account).where(Account.phone == acc_data['phone'])); ex = ex.scalar_one_or_none()
                    if ex: ex.session_string = r['session_string']; ex.is_verified = True; ex.is_sold = False
                    else: session.add(Account(phone=acc_data['phone'], session_string=r['session_string'], is_verified=True, is_sold=False))
                    await session.commit()
                del dp.awaiting_accounts[user_id]
                await message.answer(f'{pe("check")} <b>Аккаунт добавлен!</b>\n\n{pe("tag")} Номер: <code>{acc_data["phone"]}</code>\n<i>Доступен для покупки</i>', reply_markup=admin_keyboard())
            ...
            r = await verify_2fa_password(acc_data['phone'], text)
            if r['success']:
                async with async_session() as session:
                    ex = await session.execute(select(Account).where(Account.phone == acc_data['phone'])); ex = ex.scalar_one_or_none()
                    if ex: ex.session_string = r['session_string']; ex.is_verified = True; ex.is_sold = False
...
async def main():
    ...
    for attr in ['pending_accounts', 'awaiting_deposit', 'awaiting_accounts', 'awaiting_broadcast', 'awaiting_balance']:
        if not hasattr(dp, attr): setattr(dp, attr, {} if attr != 'awaiting_broadcast' else set())
