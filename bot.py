...
async def send_media_message(
    ...
    """Отправляет сообщение с медиа если оно настроено"""
    media = await get_media(section)
    
    if media and not edit:
        caption = f"{text}\n\n{media.caption}" if media.caption else text
        if media.file_type == "photo":
            await target.answer_photo(media.file_id, caption=caption, reply_markup=reply_markup)
        elif media.file_type == "video":
            await target.answer_video(media.file_id, caption=caption, reply_markup=reply_markup)
        elif media.file_type == "animation":
            await target.answer_animation(media.file_id, caption=caption, reply_markup=reply_markup)
        else:
            await target.answer(text, reply_markup=reply_markup)
    elif media and edit:
        # При edit не можем отправить медиа, только текст
...
async def check_yoomoney_payment(payment_id: str) -> dict:
    ...
    if not CLIENT_ID:
        ...
        return {'success': False, 'message': 'CLIENT_ID не настроен в .env файле'}
    ...
    try:
        ...
        logger.info(f"Checking YooMoney payment {payment_id} with CLIENT_ID: {CLIENT_ID[:10]}...")
        
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, data=data, timeout=30) as response:
                status_code = response.status
                response_text = await response.text()
                logger.info(f"YooMoney API status: {status_code}, response: {response_text[:200]}")
                
                if status_code == 401:
                    return {
                        'success': False,
                        'message': 'Ошибка авторизации. Проверьте CLIENT_ID. '
                                   'Получите токен тут: https://yoomoney.ru/api/token'
                    }
                
                if status_code != 200:
                    return {
                        'success': False,
                        'message': f'Ошибка API: {status_code}. {response_text[:100]}'
                    }
                
                try:
                    result = await response.json()
                except:
                    return {'success': False, 'message': 'Неверный ответ от API'}
                
                if "error" in result:
                    return {
                        'success': False,
                        'message': f'Ошибка: {result.get("error", "неизвестная")}'
                    }
                ...
                if not operations:
                    return {
                        'success': False,
                        'message': 'Платеж не найден. Убедитесь что вы перевели средства на кошелек.'
                    }
                
                for op in operations:
                    if op.get("label") == payment_id:
                        status = op.get("status", "")
                        amount_op = op.get("amount", 0)
                        if status == "success":
                            return {
                                'success': True,
                                'message': f'Платеж найден: {amount_op}₽'
                            }
                        else:
                            return {
                                'success': False,
                                'message': f'Статус платежа: {status}. Ожидайте подтверждения.'
                            }
                
                return {
                    'success': False,
                    'message': f'Платеж с ID {payment_id} не найден в последних 30 операциях.'
                }
        ...
        return {'success': False, 'message': f'Ошибка проверки: {str(e)[:100]}'}
...
async def callback_main_menu(callback: CallbackQuery):
    ...
    await send_media_message(callback, "main_menu", text, main_menu_keyboard(), edit=True)
...
async def callback_buy_account(callback: CallbackQuery):
    ...
    await send_media_message(callback, "buy_account", text, countries_keyboard(), edit=True)
...
async def callback_show_payment_methods(callback: CallbackQuery):
    ...
    await send_media_message(callback, "payment_methods", text, payment_methods_keyboard(), edit=True)
...
async def callback_check_purchase(callback: CallbackQuery):
    ...
    if method == "yoomoney":
        ...
        logger.info(f"Purchase check: {result}")
        ...
        if result['success']:
            ...
            if account_id:
                async with async_session() as session:
                    ...
                    if account.is_sold:
                        await callback.message.edit_text(
                            f'{pe("cross")} <b>Аккаунт уже продан</b>',
                ...
                await callback.message.edit_text(
                    f'{pe("cross")} <b>Данные утеряны</b>',
                    reply_markup=main_menu_keyboard()
                )
        else:
            await callback.message.edit_text(
                f'{pe("cross")} <b>Оплата не найдена</b>\n\n{result["message"]}\n\nПопробуйте еще раз через минуту.',
                reply_markup=check_payment_keyboard(payment_id, method, is_deposit=False)
            )
...
async def callback_check_deposit(callback: CallbackQuery):
    ...
    if method == "yoomoney":
        result = await check_yoomoney_payment(payment_id)
        logger.info(f"Deposit check: {result}")
        ...
        if result['success']:
            async with async_session() as session:
                ...
                if payment and payment.status != "completed":
                    ...
                    await callback.message.edit_text(
                        f'{pe("info")} <b>Платеж уже зачислен</b>',
                        reply_markup=main_menu_keyboard()
                    )
                else:
                    await callback.message.edit_text(
                        f'{pe("cross")} <b>Платеж не найден в базе</b>',
                        reply_markup=main_menu_keyboard()
                    )
        else:
            await callback.message.edit_text(
                f'{pe("cross")} <b>Пополнение не найдено</b>\n\n{result["message"]}\n\nПопробуйте еще раз через минуту.',
                reply_markup=check_payment_keyboard(payment_id, method, is_deposit=True)
            )
...
async def callback_my_purchases(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        if purchases:
            ...
            builder.row(InlineKeyboardButton(text="В меню", callback_data="main_menu", icon_custom_emoji_id=PREMIUM_EMOJI_IDS['home']))
            await send_media_message(callback, "my_purchases", text, builder.as_markup(), edit=True)
        else:
            text = f'{pe("box")} <b>Мои покупки</b>\n\nУ вас пока нет покупок.'
            await send_media_message(callback, "my_purchases", text, main_menu_keyboard(), edit=True)
...
async def callback_profile(callback: CallbackQuery):
    ...
    await send_media_message(callback, "profile", text, profile_keyboard(), edit=True)
...
async def callback_deposit_balance(callback: CallbackQuery):
    ...
    await send_media_message(callback, "deposit", text, deposit_keyboard(), edit=True)
...
async def callback_admin(callback: CallbackQuery, state: FSMContext):
    ...
    if data == "admin_stats":
        ...
        async with async_session() as session:
            ...
            await session.commit()
        await callback.message.edit_text(
            f'{pe("check")} <b>Все медиа удалены!</b>',
            reply_markup=admin_keyboard()
        )
