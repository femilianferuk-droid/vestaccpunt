...
EMOJI_ID = {
    ...
    'session': '5870528606328852614', 'auto': '5345906554510012647',
}
...
# ===== TELETHON =====
async def create_telethon_client(session_string: str = None, session_file: str = None):
    if session_string:
...
async def verify_session_file(file_content: bytes) -> dict:
    ...
    try:
        ...
        logger.info(f"Session saved to: {tmp_path}, size: {len(file_content)} bytes")
        ...
        if not await client.is_user_authorized():
            ...
            return {'success': False, 'error': 'Сессия не авторизована. Файл поврежден или недействителен.'}
        ...
        if not phone:
            await client.disconnect()
            return {'success': False, 'error': 'Не удалось определить номер телефона в сессии.'}
        ...
        await client.disconnect()
        logger.info(f"Session verified: {phone}, country: {country}")
        return {
            'success': True, 'phone': phone, 'country': country,
            'price': price, 'session_string': session_string,
...
async def process_purchase(user_id: int, account_id: int, price: float, method: str) -> Optional[Purchase]:
    async with async_session() as s:
        ...
        purchase = Purchase(user_id=user_id, account_id=account_id, amount=price, payment_method=method)
        s.add(purchase)
        await s.commit()
...
async def auto_check_crypto_payment(payment_id: str, user_id: int, account_id: int = None, amount: float = 0):
    logger.info(f"Auto-check started: {payment_id}")
    for i in range(120):
        ...
        try:
            ...
            if inv and inv.get("status") == "paid":
                async with async_session() as s:
                    pr = await s.execute(select(Payment).where(Payment.payment_id == payment_id))
                        ...
                        if user:
                            ...
                            try:
                                ...
                                await bot.send_message(
                                    ...
                                    f'{emoji("money")} +{payment.amount:.2f}₽\n'
...
async def cb_pay_balance(callback: CallbackQuery):
    ...
    if not aid:
        ...
        return
    if user.balance >= price:
        purchase = await process_purchase(callback.from_user.id, aid, price, "balance")
        if purchase:
            async with async_session() as s:
                acc = await s.get(Account, aid)
                await callback.message.answer(
                    f'{emoji("check")} <b>Оплата успешна!</b>\n\n{emoji("tag")} Номер: <code>{acc.phone}</code>\n{emoji("money")} Сумма: <b>{price:.0f}₽</b>',
                    reply_markup=get_code_keyboard(purchase.id)
                )
        else:
            await callback.message.answer(f'{emoji("cross")} <b>Ошибка обработки</b>', reply_markup=main_menu_keyboard())
    else:
        await callback.message.answer(
    ...
    if invoice and invoice.get("ok"):
        ...
        async with async_session() as s:
            ...
            if p:
                ...
                await s.commit()
        await callback.message.answer(
            f'{emoji("crypto")} <b>Оплата Crypto Bot</b>\n\nСумма: <b>{price:.0f}₽</b>\n\n<a href="{r.get("pay_url")}">💳 Нажмите для оплаты</a>\n\n{emoji("auto")} <b>Проверяется автоматически!</b>',
            reply_markup=check_crypto_keyboard(inv_id),
            disable_web_page_preview=True
        )
        asyncio.create_task(auto_check_crypto_payment(inv_id, callback.from_user.id, aid, price))
    else:
        ...
        if aid:
            async with async_session() as s:
                ...
                if payment and payment.status == "completed":
                    ...
                    return
                purchase = await process_purchase(callback.from_user.id, aid, price, "crypto")
                if purchase:
                    if payment:
                        payment.status = "completed"
                    await s.commit()
...
async def cb_get_session(callback: CallbackQuery):
    ...
    async with async_session() as s:
        ...
        await callback.message.answer_document(
            BufferedInputFile(account.session_string.encode(), filename=f"{account.phone}.session"),
    ...
    async with async_session() as s:
        ...
        if not account or not account.session_json:
            if account and account.session_string:
                ...
                await callback.answer("Нет JSON", show_alert=True)
                return
        await callback.message.answer_document(
            BufferedInputFile((account.session_json or "{}").encode(), filename=f"{account.phone}_session.json"),
            caption=f'{emoji("json")} JSON для {account.phone}'
        )
        b = InlineKeyboardBuilder()
        b.row(btn("В меню", callback_data="main_menu", style="default", icon="home"))
        ...
        if purchases:
            ...
            for p in purchases:
                ...
                b.row(
                    ...
                    btn("JSON", callback_data=f"get_json_{p.id}", style="default", icon="json")
                )
            b.row(btn("В меню", callback_data="main_menu", style="default", icon="home"))
...
async def cb_admin(callback: CallbackQuery, state: FSMContext):
    ...
    if data == "admin_stats":
        ...
        b = InlineKeyboardBuilder()
        b.row(btn("Отмена", callback_data="admin", style="danger", icon="cross"))
        await callback.message.answer(f'{emoji("session")} <b>Добавление через .session</b>\n\nОтправьте файл .session\nБот определит страну и цену.', reply_markup=b.as_markup())
...
async def cb_channel_del(callback: CallbackQuery):
    ...
    async with async_session() as s:
        ...
        if ch:
            await s.delete(ch)
            await s.commit()
            await callback.message.answer(f'{emoji("check")} <b>Удален!</b>', reply_markup=channels_admin_keyboard())
...
async def h_session_file(message: Message, state: FSMContext):
    ...
    if not message.document.file_name or not message.document.file_name.endswith('.session'):
        await message.answer(f'{emoji("cross")} <b>Отправьте файл .session!</b>', reply_markup=admin_keyboard())
        return
    status_msg = await message.answer(f'{emoji("loading")} <b>Проверяю сессию...</b>')
    ...
    if result['success']:
        async with async_session() as s:
            ex = await s.execute(select(Account).where(Account.phone == result['phone']))
            ex = ex.scalar_one_or_none()
            if ex:
                ex.session_string = result['session_string']
                ex.session_json = result['session_json']
                ex.is_verified = True
                ex.is_sold = False
                ex.country = result['country']
                ex.price = result['price']
            else:
                s.add(Account(
                    phone=result['phone'], country=result['country'], price=result['price'],
                    session_string=result['session_string'], session_json=result['session_json'],
                    is_verified=True, is_sold=False
                ))
            await s.commit()
        await message.answer(
            f'{emoji("check")} <b>Аккаунт добавлен!</b>\n\n{emoji("tag")} Номер: <code>{result["phone"]}</code>\n{emoji("location")} Страна: <b>{result["country"]}</b>\n{emoji("money")} Цена: <b>{result["price"]:.0f}₽</b>',
            reply_markup=admin_keyboard()
        )
    else:
        await message.answer(f'{emoji("cross")} <b>Ошибка:</b> {result["error"]}', reply_markup=admin_keyboard())
...
async def h_add_channel(message: Message, state: FSMContext):
    ...
    username = text.replace('@', '').replace('https://t.me/', '').strip('/').split()[0]
    ...
    try:
        ...
        async with async_session() as s:
            ...
            if existing.scalar_one_or_none():
                await message.answer(f'{emoji("cross")} <b>Уже добавлен!</b>', reply_markup=admin_keyboard())
                return
            s.add(RequiredChannel(channel_id=str(chat.id), channel_url=channel_url, channel_name=chat.title or username))
            await s.commit()
        await message.answer(f'{emoji("check")} <b>Канал добавлен!</b>\n\nНазвание: <b>{chat.title}</b>', reply_markup=admin_keyboard())
    except Exception as e:
        await message.answer(f'{emoji("cross")} <b>Ошибка:</b> {str(e)[:100]}', reply_markup=admin_keyboard())
        ...
        try:
            ...
            async with async_session() as s:
                if (await s.execute(select(PromoCode).where(PromoCode.code == code))).scalar_one_or_none():
                    await message.answer(f'{emoji("cross")} <b>Существует</b>', reply_markup=admin_keyboard())
                    return
            ...
            await message.answer(f'{emoji("cross")} <b>Неверный формат чисел</b>', reply_markup=admin_keyboard())
            ...
            if method == "sbp":
                ...
                if invoice and invoice.get("ok"):
                    ...
                    await message.answer(
                        ...
                        reply_markup=deposit_crypto_check_keyboard(inv_id),
                        disable_web_page_preview=True
                    )
                    asyncio.create_task(auto_check_crypto_payment(inv_id, uid))
        except:
            ...
            if res['success']:
                async with async_session() as s:
                    ...
                    if ex:
                        ...
                        ex.session_json = res['session_json']
                        ex.is_verified = True
                        ex.is_sold = False
                        ex.country = ad['country']
                        ex.price = ad['price']
                    else:
                        s.add(Account(phone=ad['phone'], country=ad['country'], price=ad['price'], session_string=res['session_string'], session_json=res['session_json'], is_verified=True, is_sold=False))
                    await s.commit()
                del dp.awaiting_accounts[uid]
