...
async def verify_session_file(file_content: bytes) -> dict:
    ...
    Бот автоматически заходит в аккаунт и определяет страну по номеру.
    """
    try:
        # Пробуем декодировать как строку
        session_str = file_content.decode('utf-8', errors='ignore').strip()
        
        if not session_str or len(session_str) < 10:
            # Может быть бинарный файл telethon
            session_str = file_content.decode('latin-1', errors='ignore')
        ...
        if not await client.is_user_authorized():
            ...
            return {
                'success': False,
                'error': 'Сессия не авторизована. Файл поврежден или недействителен.'
            }
        ...
        logger.info(f"Session verified for {phone}, country: {country}, price: {price}")
        ...
        return {
            'success': False,
            'error': f'Ошибка проверки сессии: {str(e)[:100]}'
        }
...
async def get_code_from_session(session_string: str) -> Optional[str]:
    """
    Получает код подтверждения из чата Telegram.
    Ищет во всех диалогах сообщения с 5-значными кодами.
    """
    client = None
    try:
        ...
        # Ищем в специальных чатах (Telegram, коды, верификация)
        async for dialog in client.iter_dialogs():
            dialog_name = (dialog.name or "").lower()
            if any(keyword in dialog_name for keyword in [
        ...
        # Если не нашли, ищем во всех диалогах
...
async def process_purchase(user_id: int, account_id: int, price: float, method: str) -> Optional[Purchase]:
    ...
    async with async_session() as session:
        ...
        logger.info(f"Purchase processed: user={user_id}, account={account_id}, amount={price}, method={method}")
        return purchase
...
async def auto_check_crypto_payment(payment_id: str, user_id: int, account_id: int = None, amount: float = 0):
    ...
    for attempt in range(120):  # 120 * 5 = 600 секунд = 10 минут
        ...
        try:
            ...
            if attempt % 12 == 0 and attempt > 0:  # Каждую минуту
...
async def cb_buy_account(callback: CallbackQuery):
    ...
    if available:
        text += f'{emoji("check")} Доступные страны:'
    else:
...
async def cb_country(callback: CallbackQuery):
    ...
    if account:
        # Получаем актуальную цену для страны
        price = await get_country_price(country)
...
async def cb_pay_crypto(callback: CallbackQuery):
    ...
    await callback.message.answer(f'{emoji("loading")} <b>Создаю счет...</b>')
    ...
    if invoice and invoice.get("ok"):
        ...
        text = (
            ...
            'После оплаты бот сам выдаст аккаунт.'
        )
...
async def cb_pay_stars(callback: CallbackQuery):
    ...
    text = (
        ...
        '<i>В сообщении укажите страну и количество аккаунтов</i>'
    )
    await callback.message.answer(text, reply_markup=payment_methods_keyboard())
...
async def cb_get_code(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        status_msg = await callback.message.answer(
            f'{emoji("loading")} <b>Получаю код...</b>\n\nПожалуйста, подождите несколько секунд.'
        )
...
async def cb_get_session(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        await callback.message.answer_document(
            ...
            caption=f'{emoji("file")} .session файл для {account.phone}'
        )
    ...
    async with async_session() as session:
        ...
        if purchases:
            ...
            for purchase in purchases:
                ...
                text += f'📱 <code>{phone}</code> • {purchase.amount:.0f}₽ • {date}\n'
...
async def cb_deposit_balance(callback: CallbackQuery):
    ...
    text = (
        ...
        '<i>Минимальная сумма пополнения: 10₽</i>'
    )
...
async def cb_deposit_sbp(callback: CallbackQuery):
    ...
    await callback.message.answer(
        f'{emoji("sbp")} <b>Введите сумму пополнения (от 10₽)</b>\n\n'
...
async def cb_deposit_crypto(callback: CallbackQuery):
    ...
    await callback.message.answer(
        f'{emoji("crypto")} <b>Введите сумму пополнения (от 10₽)</b>\n\n'
        '<i>Отправьте число в чат</i>'
    )
...
async def cb_admin_return(callback: CallbackQuery):
    ...
    await callback.message.answer(
        f'{emoji("stats")} <b>Админ-панель</b>',
        reply_markup=admin_keyboard()
    )
...
async def cb_admin(callback: CallbackQuery, state: FSMContext):
    ...
    if data == "admin_stats":
        async with async_session() as session:
            ...
            accounts_count = (await session.execute(select(func.count(Account.id)))).scalar() or 0
            ...
            text = (
                ...
                f'{emoji("profile")} Пользователей: <b>{users_count}</b>\n'
                f'{emoji("box")} Аккаунтов: <b>{accounts_count}</b>\n'
                f'{emoji("check")} Верифицировано: <b>{verified_count}</b>\n'
                f'{emoji("buy")} Продано: <b>{sold_count}</b>\n'
                f'{emoji("box")} Покупок: <b>{purchases_count}</b>\n'
    ...
    # --- Аккаунты ---
        ...
        builder = InlineKeyboardBuilder()
        builder.row(create_button("Удалить аккаунт", callback_data="admin_delete_account", style="danger", icon="delete"))
        builder.row(create_button("Назад", callback_data="admin", style="danger", icon="back"))
        await callback.message.answer(text, reply_markup=builder.as_markup())
    ...
    # --- Удаление аккаунта ---
        ...
        await callback.message.answer(
            f'{emoji("delete")} <b>Удаление аккаунта</b>\n\n'
    ...
    # --- Медиа ---
        ...
        await callback.message.answer(
            f'{emoji("media")} <b>Управление медиа</b>\n\n'
...
async def cb_promo_create(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        '<i>КОД - промокод, СУММА - сумма в ₽, КОЛВО - количество активаций</i>',
        reply_markup=builder.as_markup()
    )
...
async def cb_set_price(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        f'{emoji("edit")} <b>Изменение цены: {country}</b>\n\n'
...
async def h_session_file(message: Message, state: FSMContext):
    ...
    status_msg = await message.answer(
        ...
        'Загружаю файл и подключаюсь к аккаунту...'
    )
    ...
    if result['success']:
        ...
        async with async_session() as session:
            existing = await session.execute(
                select(Account).where(Account.phone == result['phone'])
            )
            ...
            if existing_account:
                ...
                existing_account.country = result['country']
                existing_account.price = result['price']
            else:
                session.add(Account(
                    phone=result['phone'],
        ...
        await message.answer(
            ...
            f'{emoji("tag")} Номер: <code>{result["phone"]}</code>\n'
            f'{emoji("location")} Страна: <b>{result["country"]}</b>\n'
            f'{emoji("money")} Цена: <b>{result["price"]:.0f}₽</b>\n'
...
async def h_text(message: Message):
    ...
    if hasattr(dp, 'awaiting_deposit') and user_id in dp.awaiting_deposit:
        ...
        try:
            ...
            if amount < 10:
                await message.answer(
                    f'{emoji("cross")} <b>Минимальная сумма: 10₽</b>',
            ...
            if method == "sbp":
                ...
                await message.answer(
                    ...
                    f'{emoji("money")} Сумма: <b>{amount}₽</b>\n\n'
                ...
                # Создаем счет
                status_msg = await message.answer(f'{emoji("loading")} Создаю счет...')
    ...
    if hasattr(dp, 'awaiting_delete_account') and user_id in dp.awaiting_delete_account:
        ...
        async with async_session() as session:
            ...
            if account:
                ...
                await message.answer(
                    f'{emoji("cross")} <b>Аккаунт не найден</b>',
                    reply_markup=admin_keyboard()
                )
        return
    ...
    if hasattr(dp, 'awaiting_accounts') and user_id in dp.awaiting_accounts:
        ...
        if step == 'phone':
            ...
            status_msg = await message.answer(
                f'{emoji("loading")} Отправляю код на {phone}...\n\n'
                f'Страна: <b>{country}</b> | Цена: <b>{price:.0f}₽</b>'
            )
        ...
        # Шаг 2: Код подтверждения
            ...
            status_msg = await message.answer(f'{emoji("loading")} Проверяю код...')
        ...
        # Шаг 3: 2FA пароль
            ...
            status_msg = await message.answer(f'{emoji("loading")} Проверяю пароль...')
    ...
    # ===== ОБЫЧНОЕ СООБЩЕНИЕ =====
    await message.answer(
        f'{emoji("info")} <b>Используйте кнопки меню для навигации</b>',
        reply_markup=main_menu_keyboard()
    )
