...
async def run_migrations():
    ...
    try:
        async with engine.begin() as conn:
            # Проверяем и добавляем колонку total_spent в users
            try:
                await conn.execute(sa_text(
                    "ALTER TABLE users ADD COLUMN IF NOT EXISTS total_spent FLOAT DEFAULT 0.0"
                ))
                logger.info("Migration: added total_spent to users")
            except Exception as e:
                logger.info(f"Migration users.total_spent: {e}")
            
            # Проверяем и добавляем колонку session_string в accounts
            try:
                await conn.execute(sa_text(
                    "ALTER TABLE accounts ADD COLUMN IF NOT EXISTS session_string TEXT"
                ))
                logger.info("Migration: added session_string to accounts")
            except Exception as e:
                logger.info(f"Migration accounts.session_string: {e}")
            
            # Проверяем и добавляем колонку is_verified в accounts
            try:
                await conn.execute(sa_text(
                    "ALTER TABLE accounts ADD COLUMN IF NOT EXISTS is_verified BOOLEAN DEFAULT FALSE"
                ))
                logger.info("Migration: added is_verified to accounts")
            except Exception as e:
                logger.info(f"Migration accounts.is_verified: {e}")
            
            # Проверяем и добавляем колонку payment_method в purchases
            try:
                await conn.execute(sa_text(
                    "ALTER TABLE purchases ADD COLUMN IF NOT EXISTS payment_method VARCHAR(50)"
                ))
                logger.info("Migration: added payment_method to purchases")
            except Exception as e:
                logger.info(f"Migration purchases.payment_method: {e}")
            
            # Проверяем и добавляем колонку code_sent в purchases
            try:
                await conn.execute(sa_text(
                    "ALTER TABLE purchases ADD COLUMN IF NOT EXISTS code_sent BOOLEAN DEFAULT FALSE"
                ))
                logger.info("Migration: added code_sent to purchases")
            except Exception as e:
                logger.info(f"Migration purchases.code_sent: {e}")
    ...
    if success:
        ...
        if account_id:
            async with async_session() as session:
                ...
                user = await session.get(User, callback.from_user.id) if hasattr(User, 'id') else await get_user(callback.from_user.id)
                if user:
                    user.total_spent = (user.total_spent or 0) + price
