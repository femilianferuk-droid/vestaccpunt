...
async def get_code_from_session(session_string: str) -> Optional[str]:
    """Получает код из чата Telegram (ищет во всех диалогах сообщения с кодами)"""
    client = None
    try:
        ...
        async for dialog in client.iter_dialogs():
            ...
            # Проверяем разные варианты имени чата
            ...
            if is_target:
                ...
                for msg in messages:
                    if msg.text:
                        logger.info(f"Message text: {msg.text[:200]}")
                        # Ищем 5-значный код
        ...
        # Если не нашли в специальных чатах, ищем везде
...
async def get_available_account():
    async with async_session() as session:
        r = await session.execute(select(Account).where(
...
async def cb_buy_account(callback: CallbackQuery):
    ...
    text += f'{pe("check")} Аккаунты в наличии' if account else f'{pe("cross")} Нет доступных'
    ...
    async with async_session() as session:
        ...
        if purchases:
            ...
            for p in purchases:
                ...
                status = "✅" if p.code_sent else "⏳"
                phone = account.phone if account else "Н/Д"
                date = p.created_at.strftime('%d.%m.%y')
                text += f'{status} <code>{phone}</code> • {p.amount}₽ • {date}\n'
                if not p.code_sent:
...
async def h_sbp_screenshot(message: Message, state: FSMContext):
    ...
    await message.answer(
        f'{pe("check")} <b>Скриншот отправлен на проверку!</b>\n\n'
...
# ===== АДМИН СБП ОДОБРЕНИЕ/ОТКЛОНЕНИЕ =====
            ...
            if user:
                ...
                try:
                    await bot.send_message(
                        ...
                        f'{pe("wallet")} Ваш баланс: <b>{new_balance:.0f}₽</b>\n\n'
...
async def cb_admin_sbp(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        if payments:
            await callback.message.answer(f'{pe("sbp")} <b>Загружаю платежи на проверку...</b>')
                ...
                try:
                    await bot.send_photo(
                        ...
                        caption=(
                            ...
                            f'@{user.username or "нет"}\n'
                            f'{pe("money")} Сумма: <b>{payment.amount}₽</b>\n'
                            f'{pe("wallet")} Баланс пользователя: <b>{user.balance:.0f}₽</b>\n'
                            f'{pe("info")} ID платежа: <code>{payment.payment_id}</code>'
                        ),
    ...
    if data == "admin_stats":
        async with async_session() as session:
            ...
            text = (
                ...
                f'{pe("profile")} Пользователей: <b>{users_cnt}</b>\n'
                f'{pe("box")} Аккаунтов: <b>{accs_cnt}</b>\n'
                f'{pe("check")} Верифицировано: <b>{verif_cnt}</b>\n'
                f'{pe("buy")} Продано: <b>{sold_cnt}</b>\n'
                f'{pe("box")} Покупок: <b>{purch_cnt}</b>\n'
                f'{pe("money")} Выручка: <b>{revenue:.0f}₽</b>'
            )
...
async def h_text(message: Message, state: FSMContext):
    ...
    if hasattr(dp, 'awaiting_accounts') and user_id in dp.awaiting_accounts:
        ...
        if step == 'phone':
            acc_data['phone'] = text
            await message.answer(f'{pe("loading")} Отправляю код на {text}...')
            r = await send_code_to_phone(text)
            if r['success']:
                acc_data['phone_code_hash'] = r['phone_code_hash']; acc_data['step'] = 'code'
                await message.answer(f'{pe("check")} <b>Код отправлен на {text}</b>\n\nВведите код из Telegram:')
            else:
            ...
            if r['success']:
                async with async_session() as session:
                    ex = await session.execute(select(Account).where(Account.phone == acc_data['phone']))
                    ex = ex.scalar_one_or_none()
                    if ex:
                        ex.session_string = r['session_string']; ex.is_verified = True; ex.is_sold = False
                    else:
                        session.add(Account(phone=acc_data['phone'], session_string=r['session_string'], is_verified=True, is_sold=False))
                    await session.commit()
                del dp.awaiting_accounts[user_id]
                await message.answer(
                    f'{pe("check")} <b>Аккаунт добавлен!</b>\n\n'
                    f'{pe("tag")} Номер: <code>{acc_data["phone"]}</code>\n'
                    f'{pe("check")} Статус: верифицирован\n'
                    f'<i>Доступен для покупки</i>',
                    reply_markup=admin_keyboard()
                )
            elif r.get('need_password'):
                acc_data['step'] = 'password'
                await message.answer(f'{pe("lock")} <b>Требуется 2FA пароль</b>\n\nВведите пароль:')
            else:
                del dp.awaiting_accounts[user_id]
                await message.answer(f'{pe("cross")} <b>Ошибка:</b> {r.get("error")}', reply_markup=admin_keyboard())
        elif step == 'password':
            ...
            if r['success']:
                ...
                await message.answer(f'{pe("cross")} <b>{r.get("error")}</b>\nПопробуйте еще раз:')
        return
        ...
        if step == 'user_id':
            try:
                ...
                if not target:
                    ...
                    del dp.awaiting_balance[user_id]; return
            ...
            try:
                ...
                async with async_session() as session:
                    target = await session.execute(select(User).where(User.telegram_id == target_id))
