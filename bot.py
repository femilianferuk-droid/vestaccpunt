...
class Payment(Base):
    ...
    type = Column(String(50), default="deposit")  # deposit или purchase
    created_at = Column(DateTime, default=datetime.utcnow)
...
def check_payment_keyboard(payment_id: str, method: str, is_deposit: bool = True):
    """is_deposit=True для пополнения, False для покупки"""
    prefix = "check_deposit" if is_deposit else "check_payment"
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload, headers=headers) as response:
                if response.status == 200:
                    ...
                    logger.info(f"Crypto Bot response: {data}")
                    return data
        return None
    except Exception as e:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params, headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
                    logger.info(f"Check invoice response: {data}")
...
async def cmd_admin(message: Message):
    if message.from_user.id not in ADMIN_IDS:
        await message.answer('<b><tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Доступ запрещен</b>')
        return
...
async def callback_main_menu(callback: CallbackQuery):
    ...
    text = '<b><tg-emoji emoji-id="5873147866364514353">🏘</tg-emoji> Главное меню</b>\n\n<i>Выберите действие:</i>'
    await callback.message.edit_text(text, reply_markup=main_menu_keyboard())

@router.callback_query(F.data == "buy_account")
async def callback_buy_account(callback: CallbackQuery):
    await callback.answer()
    text = '<b><tg-emoji emoji-id="6042011682497106307">📍</tg-emoji> Выберите страну</b>'
...
async def callback_pay_balance(callback: CallbackQuery):
    ...
    if not account_id:
        await callback.message.edit_text(
            '<b><tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Ошибка. Начните заново.</b>',
    ...
    if user.balance >= price:
        ...
        text = (
            ...
            f'<tg-emoji emoji-id="5769126056262898415">👛</tg-emoji> Баланс: <b>{user.balance:.0f}₽</b>\n'
            f'<tg-emoji emoji-id="5904462880941545555">💵</tg-emoji> Нужно: <b>{price}₽</b>\n\n'
            '<i>Пополните баланс</i>'
        )
        await callback.message.edit_text(text, reply_markup=payment_methods_keyboard())
...
async def callback_check_payment(callback: CallbackQuery):
    ...
    if method == "crypto":
        ...
        logger.info(f"Invoice check result: {invoice}")
...
async def callback_check_deposit(callback: CallbackQuery):
    ...
    if method == "crypto":
        invoice = await check_crypto_bot_invoice(int(payment_id))
        logger.info(f"Deposit invoice check: {invoice}")
        if invoice and invoice.get("status") == "paid":
            success = True
            # Получаем сумму из инвойса
            crypto_amount = float(invoice.get("amount", 0))
            amount = round(crypto_amount * 90, 2)  # Конвертируем обратно в рубли
    ...
    if success:
        async with async_session() as session:
            ...
            if payment and payment.status != "completed":
                ...
                # Зачисляем на баланс
                user = await session.get(User, callback.from_user.id)
                deposit_amount = amount if amount > 0 else payment.amount
                user.balance += deposit_amount
                
                await session.commit()
                
                text = (
                    '<b><tg-emoji emoji-id="5870633910337015697">✅</tg-emoji> Баланс пополнен!</b>\n\n'
                    f'<tg-emoji emoji-id="5904462880941545555">💵</tg-emoji> Зачислено: <b>{deposit_amount:.2f}₽</b>\n'
                    f'<tg-emoji emoji-id="5769126056262898415">👛</tg-emoji> Баланс: <b>{user.balance:.2f}₽</b>'
                )
                ...
                await callback.answer("ℹ️ Этот платеж уже зачислен", show_alert=True)
            else:
                await callback.answer("❌ Платеж не найден", show_alert=True)
    else:
        await callback.answer("⏳ Пополнение не найдено. Попробуйте позже.", show_alert=True)
...
async def callback_profile(callback: CallbackQuery):
    ...
    text = (
        ...
        f'<tg-emoji emoji-id="5870994129244131212">👤</tg-emoji> @{user.username or "нет"}\n\n'
        '━━━ 💰 БАЛАНС ━━━\n'
        ...
        f'<tg-emoji emoji-id="5904462880941545555">💵</tg-emoji> Потрачено: <b>{(user.total_spent or 0):.0f}₽</b>\n'
...
async def callback_deposit_balance(callback: CallbackQuery):
    ...
    text = (
        ...
        'Выберите способ:\n\n'
        '<tg-emoji emoji-id="5904462880941545555">💵</tg-emoji> <b>ЮMoney</b>\n'
        '<tg-emoji emoji-id="5260752406890711732">👾</tg-emoji> <b>Crypto Bot</b>\n\n'
        '<i>Минимум: 10₽</i>'
    )
...
async def callback_deposit_yoomoney(callback: CallbackQuery):
    ...
    await callback.message.answer(
        '<b><tg-emoji emoji-id="5904462880941545555">💵</tg-emoji> Введите сумму (от 10₽)</b>\n\n'
...
async def callback_deposit_crypto(callback: CallbackQuery):
    ...
    await callback.message.answer(
        '<b><tg-emoji emoji-id="5260752406890711732">👾</tg-emoji> Введите сумму (от 10₽)</b>\n\n'
        '<i>Отправьте число в чат</i>'
    )
...
async def handle_text(message: Message):
    ...
    if hasattr(dp, 'awaiting_deposit') and user_id in dp.awaiting_deposit:
        ...
        try:
            ...
            if amount < 10:
                await message.answer(
                    '<b><tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Минимум: 10₽</b>',
            ...
            if method == "yoomoney":
                ...
                if payment_url:
                    await message.answer(
                        f'<b><tg-emoji emoji-id="5904462880941545555">💵</tg-emoji> Пополнение ЮMoney</b>\n\n'
                        f'Сумма: <b>{amount}₽</b>\n'
                        f'<a href="{payment_url}">💳 Нажмите для оплаты</a>\n\n'
                        '⚠️ <b>После оплаты нажмите кнопку проверки</b>',
                ...
                if invoice and invoice.get("ok"):
                    ...
                    await message.answer(
                        f'<b><tg-emoji emoji-id="5260752406890711732">👾</tg-emoji> Пополнение Crypto Bot</b>\n\n'
                        f'Сумма: <b>{amount}₽</b>\n'
                        f'<a href="{pay_url}">💳 Нажмите для оплаты</a>\n\n'
                        '⚠️ <b>После оплаты нажмите кнопку проверки</b>',
            ...
            await message.answer('<b><tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Введите число</b>')
        return
    ...
    if hasattr(dp, 'awaiting_accounts') and user_id in dp.awaiting_accounts:
        ...
        if step == 'phone':
            ...
            if result['success']:
                ...
                await message.answer(
                    '<b><tg-emoji emoji-id="5870633910337015697">✅</tg-emoji> Аккаунт добавлен!</b>\n\n'
                ...
                await message.answer(
                    '<b><tg-emoji emoji-id="6037249452824072506">🔒</tg-emoji> Введите 2FA пароль:</b>'
                )
            else:
                del dp.awaiting_accounts[user_id]
                await message.answer(
                    f'<b><tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> {result.get("error")}</b>',
                    reply_markup=admin_keyboard()
                )
    ...
    await message.answer(
        '<b><tg-emoji emoji-id="6028435952299413210">ℹ</tg-emoji> Используйте кнопки меню</b>',
        reply_markup=main_menu_keyboard()
    )
