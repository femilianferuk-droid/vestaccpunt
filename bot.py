...
def emoji(name: str) -> str:
    eid = EMOJI_ID.get(name, EMOJI_ID['info'])
...
def btn(text: str, callback_data: str = None, url: str = None, style: str = None, icon: str = None) -> InlineKeyboardButton:
    ...
    if url:
        kwargs['url'] = url
    if style in ['primary', 'success', 'danger', 'default']:
        kwargs['style'] = style
    if icon and icon in EMOJI_ID:
        kwargs['icon_custom_emoji_id'] = EMOJI_ID[icon]
...
def main_menu_keyboard():
    ...
    b.row(
        btn("Купить аккаунт", callback_data="buy_account", style="primary", icon="buy"),
        btn("Мои покупки", callback_data="my_purchases", style="default", icon="box")
    )
    b.row(
        btn("Профиль", callback_data="profile", style="default", icon="profile"),
        btn("Пополнить", callback_data="deposit_balance", style="success", icon="wallet")
    )
    return b.as_markup()
...
async def countries_keyboard():
    ...
    for i, country in enumerate(["США", "Россия", "Индия"]):
        ...
        b.row(btn(f"{flags[country]} {country} • {price:.0f}₽", callback_data=f"country_{country}", style=styles[i], icon="location"))
...
def get_code_keyboard(pid: int):
    ...
    b.row(btn("Получить TDATA", callback_data=f"get_tdata_{pid}", style="default", icon="download"))
...
def admin_keyboard():
    ...
    for text, cb, style, icon in btns:
        b.row(btn(text, callback_data=cb, style=style, icon=icon))
...
async def price_settings_keyboard():
    ...
    prices = await get_all_prices()
    flags = {"США": "🇺🇸", "Россия": "🇷🇺", "Индия": "🇮🇳"}
    for c in ["США", "Россия", "Индия"]:
        b.row(btn(f"{flags[c]} {c}: {prices.get(c, 20):.0f}₽", callback_data=f"set_price_{c}", style="default", icon="edit"))
    b.row(btn("Назад", callback_data="admin", style="danger", icon="back"))
    return b.as_markup()

def media_menu_keyboard():
    b = InlineKeyboardBuilder()
    for name, cb in [("Главное меню", "main_menu"), ("Покупка", "buy_account"), ("Оплата", "payment_methods"), ("Профиль", "profile"), ("Покупки", "my_purchases"), ("Пополнение", "deposit")]:
        b.row(btn(name, callback_data=f"set_media_{cb}", style="default", icon="media"))
...
def sbp_approve_keyboard(pid: str, uid: int):
    b = InlineKeyboardBuilder()
    b.row(
        btn("Одобрить", callback_data=f"sbp_approve_{pid}_{uid}", style="success", icon="accept"),
        btn("Отклонить", callback_data=f"sbp_reject_{pid}_{uid}", style="danger", icon="reject")
    )
    return b.as_markup()
...
async def check_subscription(uid: int):
    ...
    for ch in chs:
        try:
            cid = ch.channel_id
            if not cid.startswith("-100") and cid.lstrip('-').isdigit():
                cid = f"-100{cid}"
...
async def get_subscribe_keyboard(ns: list):
    b = InlineKeyboardBuilder()
    for ch in ns:
        b.row(btn(f"📢 {ch.channel_name or 'Канал'}", url=ch.channel_url, style="primary", icon="subscribe"))
...
async def set_media(section: str, fid: str, ftype: str, caption: str = None):
    async with async_session() as s:
        ...
        if m:
            ...
            s.add(MediaSettings(section=section, file_id=fid, file_type=ftype, caption=caption))
        await s.commit()
...
async def send_media_message(target, section: str, text: str, markup):
    ...
    if media:
        ...
        if media.file_type == "photo":
            await msg.answer_photo(media.file_id, caption=cap, reply_markup=markup)
        elif media.file_type == "video":
            await msg.answer_video(media.file_id, caption=cap, reply_markup=markup)
        elif media.file_type == "animation":
            await msg.answer_animation(media.file_id, caption=cap, reply_markup=markup)
        else:
...
async def verify_code_and_create_all(phone: str, code: str, phash: str) -> dict:
    try:
        ...
        logger.info(f"Session+TDATA created for {phone}, TDATA size: {len(tdata_zip_base64)} chars")
...
async def get_code_from_session(ss: str) -> Optional[str]:
    client = None
    try:
        client = await create_telethon_client(ss)
        await client.connect()
...
async def get_or_create_user(uid: int, username: str = None):
    ...
    if not u:
        async with async_session() as s:
            u = User(telegram_id=uid, username=username, is_admin=(uid in ADMIN_IDS))
            s.add(u)
            await s.commit()
            await s.refresh(u)
    return u
...
async def cb_check_sub(callback: CallbackQuery):
    ...
    if subbed:
        await callback.message.answer(f'{emoji("check")} <b>Подписка проверена!</b>', reply_markup=main_menu_keyboard())
    else:
        await callback.message.answer(f'{emoji("cross")} <b>Вы не подписаны!</b>', reply_markup=await get_subscribe_keyboard(ns))
...
async def cb_buy_account(callback: CallbackQuery):
    ...
    text += f'{emoji("check")} Доступные страны:' if avail else f'{emoji("cross")} Нет аккаунтов'
...
async def cb_country(callback: CallbackQuery):
    ...
    if account:
        ...
        await callback.message.answer(
            ...
            f'{emoji("location")} Страна: {flags.get(country, "")} <b>{country}</b>\n'
            f'{emoji("money")} Цена: <b>{price:.0f}₽</b>\n\n'
            '<i>Нажмите КУПИТЬ</i>',
            reply_markup=account_found_keyboard()
        )
    else:
        await callback.message.answer(f'{emoji("cross")} <b>Нет аккаунтов для {country}</b>', reply_markup=await countries_keyboard())
...
async def cb_pay_balance(callback: CallbackQuery):
    ...
    if not aid:
        await callback.message.answer(f'{emoji("cross")} <b>Ошибка</b>', reply_markup=main_menu_keyboard())
        return
    if user.balance >= price:
        async with async_session() as s:
            ...
            if account.is_sold:
                await callback.message.answer(f'{emoji("cross")} <b>Продан</b>', reply_markup=main_menu_keyboard())
                return
            user.balance -= price
...
async def cb_pay_sbp(callback: CallbackQuery):
    ...
    b = InlineKeyboardBuilder()
    b.row(btn("Пополнить баланс", callback_data="deposit_balance", style="success", icon="wallet"))
    b.row(btn("Назад", callback_data="show_payment_methods", style="default", icon="back"))
    await callback.message.answer(
        f'{emoji("sbp")} <b>Оплата через СБП</b>\n\n{emoji("info")} Для оплаты товара через СБП пополните баланс.',
...
async def cb_pay_crypto(callback: CallbackQuery):
    ...
    async with async_session() as s:
        s.add(Payment(user_id=callback.from_user.id, amount=price, payment_id=pid, method="crypto", status="pending", type="purchase"))
        await s.commit()
    invoice = await create_crypto_bot_invoice(price, pid)
    if invoice and invoice.get("ok"):
        r = invoice.get("result", {})
        async with async_session() as s:
            p = await s.execute(select(Payment).where(Payment.payment_id == pid))
            p = p.scalar_one_or_none()
            if p:
                p.payment_id = str(r.get("invoice_id"))
                await s.commit()
        await callback.message.answer(
            f'{emoji("crypto")} <b>Оплата Crypto Bot</b>\n\nСумма: <b>{price:.0f}₽</b>\n\n<a href="{r.get("pay_url")}">💳 Нажмите для оплаты</a>\n\n⚠️ После оплаты нажмите проверку',
            reply_markup=check_crypto_keyboard(str(r.get("invoice_id"))),
            disable_web_page_preview=True
        )
    else:
        await callback.message.answer(f'{emoji("cross")} <b>Ошибка</b>', reply_markup=payment_methods_keyboard())
        ...
        if aid:
            async with async_session() as s:
                ...
                if account.is_sold:
                    await callback.message.answer(f'{emoji("cross")} <b>Продан</b>', reply_markup=main_menu_keyboard())
                    return
                pr = await s.execute(select(Payment).where(Payment.payment_id == pid))
    ...
    if inv and inv.get("status") == "paid":
        async with async_session() as s:
            ...
            if payment and payment.status != "completed":
                ...
                await s.commit()
                b = InlineKeyboardBuilder()
                b.row(btn("В меню", callback_data="main_menu", style="success", icon="home"))
                await callback.message.answer(
                    f'{emoji("check")} <b>Баланс пополнен!</b>\n\n{emoji("money")} +{payment.amount:.2f}₽\n{emoji("wallet")} Баланс: <b>{user.balance:.2f}₽</b>',
                    reply_markup=b.as_markup()
                )
    else:
        await callback.answer("⏳ Не найдено", show_alert=True)
...
async def cb_get_code(callback: CallbackQuery):
    ...
    async with async_session() as s:
        r = await s.execute(select(Purchase).where(Purchase.id == pid))
        ...
        if code:
            ...
            b.row(btn("В меню", callback_data="main_menu", style="default", icon="home"))
            await callback.message.answer(
                f'{emoji("check")} <b>Код: <code>{code}</code></b>\n\n{emoji("tag")} Номер: <code>{account.phone}</code>',
                reply_markup=b.as_markup()
            )
        else:
            await callback.message.answer(f'{emoji("cross")} <b>Не удалось</b>', reply_markup=get_code_keyboard(pid))
...
async def cb_get_tdata(callback: CallbackQuery):
    await callback.answer()
    pid = int(callback.data.replace("get_tdata_", ""))
    logger.info(f"TDATA requested for purchase {pid}")
    ...
    async with async_session() as s:
        ...
        if not purchase or purchase.user_id != callback.from_user.id:
            logger.error(f"Purchase not found or wrong user: {pid}")
            await callback.answer("Покупка не найдена", show_alert=True)
            return
        
        account = await s.get(Account, purchase.account_id)
        if not account:
            logger.error(f"Account not found for purchase {pid}")
            await callback.answer("Аккаунт не найден", show_alert=True)
            return
        
        if not account.tdata_zip_base64:
            logger.error(f"No TDATA for account {account.phone} (tdata_zip_base64 is None or empty)")
            await callback.answer("TDATA отсутствует. Попробуйте получить .session файл.", show_alert=True)
            return
        
        try:
            # Декодируем base64
            zip_data = base64.b64decode(account.tdata_zip_base64)
            logger.info(f"TDATA decoded: {len(zip_data)} bytes for {account.phone}")
            
            # Проверяем что это валидный zip
            if not zip_data[:4] == b'PK\x03\x04':
                logger.error(f"Invalid zip data for {account.phone} (starts with {zip_data[:10]})")
                await callback.answer("TDATA поврежден. Обратитесь в поддержку.", show_alert=True)
                return
            
            # Отправляем файл
            await callback.message.answer_document(
                BufferedInputFile(zip_data, filename=f"{account.phone}_tdata.zip"),
                caption=f'{emoji("download")} TDATA для {account.phone}'
            )
            
            b = InlineKeyboardBuilder()
            b.row(btn("В меню", callback_data="main_menu", style="default", icon="home"))
            await callback.message.answer(f'{emoji("check")} <b>TDATA отправлен!</b>', reply_markup=b.as_markup())
            logger.info(f"TDATA sent successfully for {account.phone}")
        except base64.binascii.Error as e:
            logger.error(f"Base64 decode error: {e}")
            await callback.answer("Ошибка декодирования TDATA", show_alert=True)
        except Exception as e:
            logger.error(f"Error sending TDATA: {e}")
    ...
    async with async_session() as s:
        ...
        if purchases:
            ...
            for p in purchases:
                ...
                b.row(
                    btn("Код", callback_data=f"get_code_{p.id}", style="primary", icon="code"),
                    btn(".session", callback_data=f"get_session_{p.id}", style="default", icon="file"),
                    btn("TDATA", callback_data=f"get_tdata_{p.id}", style="default", icon="download")
                )
            b.row(btn("В меню", callback_data="main_menu", style="default", icon="home"))
            await send_media_message(callback, "my_purchases", text, b.as_markup())
        else:
            await send_media_message(callback, "my_purchases", f'{emoji("box")} <b>Мои покупки</b>\n\nПока нет покупок.', main_menu_keyboard())
...
async def cb_profile(callback: CallbackQuery):
    ...
    text = (
        ...
        f'{emoji("tag")} ID: <code>{user.telegram_id}</code>\n'
        f'{emoji("profile")} @{user.username or "нет"}\n\n'
        '━━━ 💰 БАЛАНС ━━━\n'
        f'{emoji("wallet")} <b>{user.balance:.0f}₽</b>\n'
        '━━━━━━━━━━━━━━\n\n'
        '━━ 📊 СТАТИСТИКА ━━\n'
        f'{emoji("box")} Покупок: <b>{cnt}</b>\n'
        f'{emoji("money")} Потрачено: <b>{(user.total_spent or 0):.0f}₽</b>\n'
        f'{emoji("clock")} С нами: {user.created_at.strftime("%d.%m.%Y")}\n'
        '━━━━━━━━━━━━━━'
    )
...
async def cb_activate_promo(callback: CallbackQuery, state: FSMContext):
    ...
    b = InlineKeyboardBuilder()
    b.row(btn("Отмена", callback_data="profile", style="danger", icon="cross"))
...
async def cb_sbp_approve(callback: CallbackQuery):
    ...
    pid, uid = parts[0], int(parts[1])
    async with async_session() as s:
        pr = await s.execute(select(Payment).where(Payment.payment_id == pid))
        payment = pr.scalar_one_or_none()
        if payment and payment.status != "completed":
            payment.status = "completed"
            user = await s.execute(select(User).where(User.telegram_id == uid))
            user = user.scalar_one_or_none()
            if user:
                old, new = user.balance, user.balance + payment.amount
                user.balance = new
                await s.commit()
                ...
                try:
                    await bot.send_message(uid, f'{emoji("check")} <b>Платеж одобрен!</b>\n\n{emoji("money")} +{payment.amount}₽\n{emoji("wallet")} Баланс: <b>{new:.0f}₽</b>')
                except:
                    pass
...
async def cb_sbp_reject(callback: CallbackQuery):
    ...
    async with async_session() as s:
        ...
        if payment:
            ...
            await s.commit()
            await callback.message.edit_caption(
                f'{callback.message.caption}\n\n{emoji("cross")} <b>ОТКЛОНЕНО</b>',
                reply_markup=None
            )
            try:
                await bot.send_message(uid, f'{emoji("cross")} <b>Платеж отклонен</b>\n\n@v3estnikov')
            except:
                pass
            ...
            for p in payments:
                ...
                try:
                    await bot.send_photo(
                        callback.from_user.id, p.screenshot_file_id,
                        caption=f'{emoji("sbp")} <b>СБП</b>\n{emoji("profile")} ID: <code>{p.user_id}</code>\n{emoji("money")} Сумма: <b>{p.amount}₽</b>\n{emoji("wallet")} Баланс: <b>{user.balance:.0f}₽</b>',
                        reply_markup=sbp_approve_keyboard(p.payment_id, p.user_id)
                    )
                except:
                    pass
            await callback.message.answer(f'{emoji("info")} Проверьте платежи', reply_markup=admin_keyboard())
        else:
            await callback.message.answer(f'{emoji("info")} <b>Нет платежей</b>', reply_markup=admin_keyboard())
    ...
    if data == "admin_stats":
        async with async_session() as s:
            ...
            text = (
                ...
                f'{emoji("profile")} Пользователей: <b>{uc}</b>\n'
                f'{emoji("box")} Аккаунтов: <b>{ac}</b>\n'
                f'{emoji("check")} Вериф: <b>{vc}</b>\n'
                f'{emoji("buy")} Продано: <b>{sc}</b>\n'
                f'{emoji("box")} Покупок: <b>{pc}</b>\n'
                f'{emoji("money")} Выручка: <b>{rev:.0f}₽</b>'
            )
        ...
        async with async_session() as s:
            ...
            for a in accounts:
                text += f'{"✅" if a.is_verified else "⏳"} <code>{a.phone}</code> | {a.country} | {a.price:.0f}₽ | {"ПРОДАН" if a.is_sold else "в наличии"}\n'
        b = InlineKeyboardBuilder()
        b.row(btn("Удалить аккаунт", callback_data="admin_delete_account", style="danger", icon="delete"))
        b.row(btn("Назад", callback_data="admin", style="danger", icon="back"))
        await callback.message.answer(text, reply_markup=b.as_markup())
        ...
        b = InlineKeyboardBuilder()
        b.row(btn("Отмена", callback_data="admin", style="danger", icon="cross"))
        ...
        await callback.message.answer(f'{emoji("settings")} <b>Цены</b>', reply_markup=await price_settings_keyboard())
    
    elif data == "admin_promo_menu":
        await callback.message.answer(f'{emoji("promo")} <b>Промокоды</b>', reply_markup=promo_admin_keyboard())
    
    elif data == "promo_create":
        await state.set_state(PromoStates.waiting_for_promo_data)
        b = InlineKeyboardBuilder()
        b.row(btn("Отмена", callback_data="admin_promo_menu", style="danger", icon="cross"))
        await callback.message.answer(f'{emoji("promo")} <b>Создание промокода</b>\n\nФормат: <code>КОД СУММА КОЛВО</code>\nПример: <code>HELLO 50 10</code>', reply_markup=b.as_markup())
        ...
        async with async_session() as s:
            ...
            for p in promos:
                text += f'<code>{p.code}</code> | {p.amount}₽ | {p.used_count}/{p.max_uses} | {"✅" if p.is_active else "❌"}\n'
            if not promos:
                text += 'Нет промокодов'
        b = InlineKeyboardBuilder()
        ...
        async with async_session() as s:
            ...
            promos = r.scalars().all()
            if not promos:
                await callback.message.answer(f'{emoji("info")} Нет промокодов', reply_markup=promo_admin_keyboard())
                return
            b = InlineKeyboardBuilder()
            for p in promos:
                b.row(btn(f"❌ {p.code}", callback_data=f"promo_delete_{p.id}", style="danger", icon="delete"))
            b.row(btn("Назад", callback_data="admin_promo_menu", style="danger", icon="back"))
        ...
        pid = int(data.replace("promo_delete_", ""))
        async with async_session() as s:
            promo = await s.get(PromoCode, pid)
            if promo:
                await s.delete(promo)
                await s.commit()
                await callback.message.answer(f'{emoji("check")} <b>Удален!</b>', reply_markup=promo_admin_keyboard())
    
    elif data == "admin_media_menu":
        await callback.message.answer(f'{emoji("media")} <b>Управление медиа</b>', reply_markup=media_menu_keyboard())
        ...
        await callback.message.answer(f'{emoji("channel")} <b>Обязательные каналы</b>', reply_markup=channels_admin_keyboard())
    
    elif data == "channel_add":
        await state.set_state(ChannelStates.waiting_for_channel)
        b = InlineKeyboardBuilder()
        b.row(btn("Отмена", callback_data="admin_channels_menu", style="danger", icon="cross"))
        await callback.message.answer(
        ...
        async with async_session() as s:
            ...
            channels = r.scalars().all()
            if not channels:
                await callback.message.answer(f'{emoji("info")} Нет каналов', reply_markup=channels_admin_keyboard())
                return
            b = InlineKeyboardBuilder()
            for ch in channels:
                b.row(btn(f"❌ {ch.channel_name or ch.channel_id}", callback_data=f"channel_del_{ch.id}", style="danger", icon="delete"))
            b.row(btn("Назад", callback_data="admin_channels_menu", style="danger", icon="back"))
            await callback.message.answer(f'{emoji("delete")} <b>Выберите для удаления:</b>', reply_markup=b.as_markup())
    
    elif data.startswith("channel_del_"):
        cid = int(data.replace("channel_del_", ""))
        async with async_session() as s:
            ch = await s.get(RequiredChannel, cid)
            if ch:
                await s.delete(ch)
                await s.commit()
                await callback.message.answer(f'{emoji("check")} <b>Канал удален!</b>', reply_markup=channels_admin_keyboard())
...
async def cb_set_price(callback: CallbackQuery, state: FSMContext):
    ...
    b = InlineKeyboardBuilder()
    b.row(btn("Отмена", callback_data="admin_prices", style="danger", icon="cross"))
    await callback.message.answer(
        f'{emoji("edit")} <b>Цена: {country}</b>\n\nТекущая: <b>{cur:.0f}₽</b>\n\nОтправьте новую цену:',
        reply_markup=b.as_markup()
    )
...
async def cb_set_media(callback: CallbackQuery, state: FSMContext):
    ...
    b = InlineKeyboardBuilder()
    b.row(btn("Отмена", callback_data="admin_media_menu", style="danger", icon="cross"))
    await callback.message.answer(
        f'{emoji("media")} <b>Установка медиа</b>\n\nРаздел: <b>{names.get(section, section)}</b>\n\nОтправьте фото/видео/GIF.',
        reply_markup=b.as_markup()
    )

# ===== ОБРАБОТЧИКИ СООБЩЕНИЙ С FSM =====
...
async def h_activate_promo(message: Message, state: FSMContext):
    ...
    async with async_session() as s:
        ...
        if not promo:
            await message.answer(f'{emoji("cross")} <b>Не найден</b>', reply_markup=profile_keyboard())
            return
        if promo.used_count >= promo.max_uses:
            await message.answer(f'{emoji("cross")} <b>Исчерпан</b>', reply_markup=profile_keyboard())
            return
        r = await s.execute(select(PromoUsage).where(PromoUsage.user_id == message.from_user.id, PromoUsage.promo_id == promo.id))
        if r.scalar_one_or_none():
            await message.answer(f'{emoji("cross")} <b>Уже использован</b>', reply_markup=profile_keyboard())
            return
        promo.used_count += 1
...
async def h_sbp_screenshot(message: Message, state: FSMContext):
    ...
    async with async_session() as s:
        ...
        payment = pr.scalar_one_or_none()
        if payment:
            payment.screenshot_file_id = fid
            await s.commit()
        ...
        if payment:
            ...
            for aid in ADMIN_IDS:
                try:
                    await bot.send_photo(
                        aid, fid,
                        caption=f'{emoji("sbp")} <b>СБП платеж</b>\n\n{emoji("profile")} ID: <code>{payment.user_id}</code>\n@{user.username or "нет"}\n{emoji("money")} Сумма: <b>{payment.amount}₽</b>\n{emoji("wallet")} Баланс: <b>{user.balance:.0f}₽</b>',
                        reply_markup=sbp_approve_keyboard(pid, payment.user_id)
                    )
                except:
                    pass
...
async def h_set_price(message: Message, state: FSMContext):
    ...
    try:
        ...
        if price <= 0:
            await message.answer(f'{emoji("cross")} <b>Цена > 0</b>', reply_markup=admin_keyboard())
            return
        ...
        await message.answer(f'{emoji("cross")} <b>Введите число</b>', reply_markup=admin_keyboard())
...
async def h_add_channel(message: Message, state: FSMContext):
    ...
    if not parts:
        await message.answer(f'{emoji("cross")} <b>Отправьте @username или ссылку</b>', reply_markup=admin_keyboard())
        return
    
    username = parts[0].replace('@', '').replace('https://t.me/', '').replace('http://t.me/', '').strip('/')
    channel_url = parts[1] if len(parts) >= 2 and parts[1].startswith('https://') else f"https://t.me/{username}"
    ...
    try:
        ...
        await message.answer(
            f'{emoji("cross")} <b>Ошибка добавления канала</b>\n\n'
            f'{str(e)}\n\n'
            'Убедитесь что:\n'
            '1. Канал существует\n'
        ...
        try:
            ...
            async with async_session() as s:
                if (await s.execute(select(PromoCode).where(PromoCode.code == code))).scalar_one_or_none():
                    await message.answer(f'{emoji("cross")} <b>Промокод {code} уже существует</b>', reply_markup=admin_keyboard())
                    return
            ...
            await message.answer(f'{emoji("cross")} <b>Неверный формат чисел</b>\nПример: <code>HELLO 50 10</code>', reply_markup=admin_keyboard())
    else:
        await message.answer(f'{emoji("cross")} <b>Неверный формат</b>\nПример: <code>HELLO 50 10</code>', reply_markup=admin_keyboard())
...
async def h_text(message: Message):
    ...
    if hasattr(dp, 'awaiting_deposit') and uid in dp.awaiting_deposit:
        ...
        try:
            ...
            if amount < 10:
                await message.answer(f'{emoji("cross")} <b>Минимум: 10₽</b>', reply_markup=deposit_keyboard())
                return
            pid = await generate_payment_id()
            if method == "sbp":
                async with async_session() as s:
                    s.add(Payment(user_id=uid, amount=amount, payment_id=pid, method="sbp", status="pending", type="deposit"))
                    await s.commit()
                await message.answer(
                    f'{emoji("sbp")} <b>Пополнение СБП</b>\n\n{emoji("money")} Сумма: <b>{amount}₽</b>\n\n'
                    f'{emoji("bank")} <b>Реквизиты:</b>\n📱 <code>{SBP_PHONE}</code>\n🏦 Банк: <b>{SBP_BANK}</b>\n👤 Получатель: <b>{SBP_RECEIVER}</b>\n\n'
                    '⚠️ Нажмите "Я оплатил"',
                    reply_markup=sbp_payment_keyboard(pid)
                )
            elif method == "crypto":
                async with async_session() as s:
                    s.add(Payment(user_id=uid, amount=amount, payment_id=pid, method="crypto", status="pending", type="deposit"))
                    await s.commit()
                ...
                if invoice and invoice.get("ok"):
                    ...
                    async with async_session() as s:
                        ...
                        if p:
                            ...
                            await s.commit()
                    await message.answer(
                        f'{emoji("crypto")} <b>Пополнение Crypto Bot</b>\n\nСумма: <b>{amount}₽</b>\n\n<a href="{r.get("pay_url")}">💳 Нажмите для оплаты</a>\n\n⚠️ Нажмите проверку',
                        reply_markup=deposit_crypto_check_keyboard(str(r.get("invoice_id"))),
                        disable_web_page_preview=True
                    )
        except:
            await message.answer(f'{emoji("cross")} <b>Введите число</b>')
        return
    ...
    if hasattr(dp, 'awaiting_delete_account') and uid in dp.awaiting_delete_account:
        ...
        async with async_session() as s:
            r = await s.execute(select(Account).where(Account.phone == text))
            account = r.scalar_one_or_none()
            if account:
                await s.delete(account)
                await s.commit()
                await message.answer(f'{emoji("check")} <b>Аккаунт {text} удален!</b>', reply_markup=admin_keyboard())
            else:
                await message.answer(f'{emoji("cross")} <b>Аккаунт не найден</b>', reply_markup=admin_keyboard())
        return
    
    # Добавление аккаунта
    if hasattr(dp, 'awaiting_accounts') and uid in dp.awaiting_accounts:
        ad = dp.awaiting_accounts[uid]
        step = ad.get('step')
        ...
        if step == 'phone':
            ...
            if res['success']:
                ad['phone_code_hash'] = res['phone_code_hash']
                ad['step'] = 'code'
                await message.answer(f'{emoji("check")} <b>Код отправлен на {phone}</b>\n\nСтрана: <b>{country}</b> | Цена: <b>{price:.0f}₽</b>\n\nВведите код:')
            else:
                del dp.awaiting_accounts[uid]
                await message.answer(f'{emoji("cross")} <b>Ошибка</b>', reply_markup=admin_keyboard())
        
        elif step == 'code':
            res = await verify_code_and_create_all(ad['phone'], text, ad['phone_code_hash'])
            if res['success']:
                async with async_session() as s:
                    ex = await s.execute(select(Account).where(Account.phone == ad['phone']))
                    ex = ex.scalar_one_or_none()
                    if ex:
                        ex.session_string = res['session_string']
                        ex.tdata_zip_base64 = res['tdata_zip_base64']
                        ex.is_verified = True
                        ex.is_sold = False
                        ex.country = ad['country']
                        ex.price = ad['price']
                    else:
                        s.add(Account(
                            phone=ad['phone'], country=ad['country'], price=ad['price'],
                            session_string=res['session_string'], tdata_zip_base64=res['tdata_zip_base64'],
                            is_verified=True, is_sold=False
                        ))
                    await s.commit()
                del dp.awaiting_accounts[uid]
                await message.answer(
                    f'{emoji("check")} <b>Аккаунт добавлен!</b>\n\n{emoji("tag")} Номер: <code>{ad["phone"]}</code>\n{emoji("location")} Страна: <b>{ad["country"]}</b>\n{emoji("money")} Цена: <b>{ad["price"]:.0f}₽</b>\n<i>Доступен для покупки</i>',
                    reply_markup=admin_keyboard()
                )
            elif res.get('need_password'):
                ad['step'] = 'password'
                await message.answer(f'{emoji("lock")} <b>Введите 2FA пароль:</b>')
            else:
                del dp.awaiting_accounts[uid]
                await message.answer(f'{emoji("cross")} <b>{res.get("error")}</b>', reply_markup=admin_keyboard())
        
        elif step == 'password':
            res = await verify_2fa_and_create_all(ad['phone'], text)
            if res['success']:
                async with async_session() as s:
                    ex = await s.execute(select(Account).where(Account.phone == ad['phone']))
                    ex = ex.scalar_one_or_none()
                ...
                await message.answer(f'{emoji("cross")} <b>{res.get("error")}</b>\nПопробуйте еще раз:')
        return
    
    # Изменение баланса
    if hasattr(dp, 'awaiting_balance') and uid in dp.awaiting_balance:
        bd = dp.awaiting_balance[uid]
        step = bd.get('step')
        if step == 'user_id':
            try:
                target = await get_user(int(text))
                if not target:
                    await message.answer(f'{emoji("cross")} <b>Не найден</b>', reply_markup=admin_keyboard())
                    del dp.awaiting_balance[uid]
                    return
                bd['target_id'] = int(text)
                bd['step'] = 'amount'
                ...
                await message.answer(f'{emoji("cross")} <b>Введите ID</b>')
        elif step == 'amount':
            try:
                tid = bd['target_id']
                async with async_session() as s:
                    target = await s.execute(select(User).where(User.telegram_id == tid))
                    ...
                    old = target.balance
                    if text.startswith('+'):
                        target.balance += float(text[1:])
                    elif text.startswith('-'):
                        target.balance = max(0, target.balance - float(text[1:]))
                    else:
                        target.balance = float(text)
                    await s.commit()
                    del dp.awaiting_balance[uid]
                    await message.answer(f'{emoji("check")} <b>Баланс изменен!</b>\n\nID: <code>{tid}</code>\nБыло: <b>{old:.0f}₽</b>\nСтало: <b>{target.balance:.0f}₽</b>', reply_markup=admin_keyboard())
            except:
                await message.answer(f'{emoji("cross")} <b>Введите сумму</b>')
        return
