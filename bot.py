...
YOMONEY_REDIRECT_URI = os.getenv("YOMONEY_REDIRECT_URI", "https://localhost")
...
class YoomoneyToken(Base):
    __tablename__ = "yoomoney_token"
    id = Column(Integer, primary_key=True)
...
class YoomoneyAuthStates(StatesGroup):
...
# ===== YOOMONEY ТОКЕН =====
async def get_yoomoney_token() -> Optional[str]:
    """Получает сохраненный токен из БД"""
    async with async_session() as session:
        result = await session.execute(
            select(YoomoneyToken).order_by(YoomoneyToken.created_at.desc()).limit(1)
        )
        token_record = result.scalar_one_or_none()
        if token_record:
            # Проверяем не истек ли (3 года)
            if token_record.expires_at and token_record.expires_at > datetime.utcnow():
                return token_record.access_token
        return None

async def save_yoomoney_token(access_token: str):
    """Сохраняет токен в БД"""
    async with async_session() as session:
        token_record = YoomoneyToken(
            access_token=access_token,
            expires_at=datetime.utcnow().replace(year=datetime.utcnow().year + 3)
        )
        session.add(token_record)
        await session.commit()
        logger.info("Yoomoney token saved")

async def exchange_code_for_token(code: str) -> Optional[str]:
    """Обменивает authorization code на access token"""
    if not YOOMONEY_CLIENT_ID:
        logger.error("YOOMONEY_CLIENT_ID not set")
        return None
    ...
    try:
        ...
        data = {
            ...
            "redirect_uri": YOMONEY_REDIRECT_URI
        }
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, data=data, timeout=30) as response:
                ...
                if "access_token" in result:
                    await save_yoomoney_token(result["access_token"])
                    return result["access_token"]
                else:
                    logger.error(f"Token exchange error: {result.get('error')}")
                    return None
    except Exception as e:
...
async def get_auth_url() -> str:
    ...
    return (
        ...
        f"redirect_uri={YOMONEY_REDIRECT_URI}&"
        f"scope=account-info%20operation-history"
    )
...
async def send_media_message(target, section: str, text: str, reply_markup: InlineKeyboardMarkup):
    """Отправляет новое сообщение с медиа если настроено"""
...
def payment_methods_keyboard():
    ...
    builder.row(InlineKeyboardButton(text="ЮMoney", callback_data="pay_yoomoney", icon_custom_emoji_id=PREMIUM_EMOJI_IDS['money']))
...
def check_payment_keyboard(payment_id: str, method: str, is_deposit: bool = True):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Проверить оплату", callback_data=f"{prefix}_{method}_{payment_id}", icon_custom_emoji_id=PREMIUM_EMOJI_IDS['loading']))
...
def deposit_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="ЮMoney", callback_data="deposit_yoomoney", icon_custom_emoji_id=PREMIUM_EMOJI_IDS['money']))
...
def admin_keyboard():
    ...
    builder.row(InlineKeyboardButton(text="YooMoney токен", callback_data="admin_yoomoney_auth", icon_custom_emoji_id=PREMIUM_EMOJI_IDS['key']))
    builder.row(InlineKeyboardButton(text="В меню", callback_data="main_menu", icon_custom_emoji_id=PREMIUM_EMOJI_IDS['home']))
    return builder.as_markup()
...
async def check_yoomoney_payment(payment_id: str) -> dict:
    """Проверяет платеж через YooMoney API"""
    token = await get_yoomoney_token()
    
    if not token:
        return {
            'success': False,
            'message': 'Токен YooMoney не настроен. Админ должен авторизоваться в админ-панели.'
        }
    ...
    try:
        ...
        data = {"label": payment_id, "records": 30}
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, data=data, timeout=30) as response:
                if response.status == 401:
                    return {
                        'success': False,
                        'message': 'Токен истек. Админ должен обновить авторизацию.'
                    }
                ...
                for op in operations:
                    if op.get("label") == payment_id:
                        if op.get("status") == "success":
                            return {'success': True, 'message': f'Платеж найден: {op.get("amount")}₽'}
                        else:
                            return {'success': False, 'message': f'Статус: {op.get("status")}'}
                
                return {'success': False, 'message': 'Платеж не найден. Проверьте перевод.'}
...
async def callback_check_purchase(callback: CallbackQuery):
    ...
    if success:
        ...
        msg = message_text or 'Оплата не найдена. Попробуйте позже.'
        await callback.message.edit_text(
            f'{pe("cross")} <b>{msg}</b>',
            reply_markup=check_payment_keyboard(payment_id, method, is_deposit=False)
        )
    ...
    if success:
        async with async_session() as session:
            ...
            if payment and payment.status != "completed":
                ...
                await callback.message.edit_text(f'{pe("info")} <b>Платеж уже зачислен</b>', reply_markup=main_menu_keyboard())
    else:
        msg = message_text or 'Пополнение не найдено.'
        await callback.message.edit_text(
            f'{pe("cross")} <b>{msg}</b>',
            reply_markup=check_payment_keyboard(payment_id, method, is_deposit=True)
        )
...
async def callback_deposit_yoomoney(callback: CallbackQuery):
    await callback.answer()
    await callback.message.answer(f'{pe("money")} <b>Введите сумму (от 10₽)</b>\n\n<i>Отправьте число в чат</i>')
...
async def callback_admin(callback: CallbackQuery, state: FSMContext):
    ...
    if data == "admin_stats":
    ...
    elif data == "admin_yoomoney_auth":
        if not YOOMONEY_CLIENT_ID:
            await callback.message.edit_text(
                f'{pe("cross")} <b>YOOMONEY_CLIENT_ID не настроен</b>\n\n'
                'Добавьте в .env:\n<code>YOOMONEY_CLIENT_ID=ваш_client_id</code>',
        ...
        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(text="Открыть ссылку", url=auth_url))
        builder.row(InlineKeyboardButton(text="Я получил код", callback_data="admin_yoomoney_code"))
        builder.row(InlineKeyboardButton(text="Назад", callback_data="admin"))
        
        await callback.message.edit_text(
            f'{pe("key")} <b>Авторизация YooMoney</b>\n\n'
            f'1. Нажмите "Открыть ссылку"\n'
            f'2. Подтвердите доступ\n'
            f'3. Скопируйте <code>code</code> из URL\n'
            f'4. Нажмите "Я получил код" и отправьте его\n\n'
            f'CLIENT_ID: <code>{YOOMONEY_CLIENT_ID[:10]}...</code>',
            reply_markup=builder.as_markup()
        )
    
    elif data == "admin_yoomoney_code":
        await state.set_state(YoomoneyAuthStates.waiting_for_code)
        
        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(text="Отмена", callback_data="admin"))
        
        await callback.message.edit_text(
            f'{pe("key")} <b>Отправьте код</b>\n\n'
            'Скопируйте <code>code</code> из URL после авторизации\n'
            'и отправьте его в чат.',
            reply_markup=builder.as_markup()
        )
...
# ===== YOOMONEY AUTH CODE =====
@router.message(StateFilter(YoomoneyAuthStates.waiting_for_code), F.text)
async def handle_yoomoney_code(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    ...
    await message.answer(f'{pe("loading")} Обмениваю код на токен...')
    
    token = await exchange_code_for_token(code)
    
    if token:
        await message.answer(
            f'{pe("check")} <b>Токен получен!</b>\n\n'
            f'Токен: <code>{token[:20]}...</code>\n'
            'Теперь проверка платежей YooMoney работает.',
            reply_markup=admin_keyboard()
        )
    else:
        await message.answer(
            f'{pe("cross")} <b>Ошибка получения токена</b>\n\n'
...
async def callback_set_media(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f'{pe("media")} <b>Установка медиа</b>\n\n'
        f'Раздел: <b>{sections_names.get(section, section)}</b>\n\n'
...
async def handle_media_upload(message: Message, state: FSMContext):
    ...
    await message.answer(
        f'{pe("check")} <b>Медиа установлено!</b>\n\n'
        f'Раздел: <b>{sections_names.get(section, section)}</b>',
        reply_markup=admin_keyboard()
    )
