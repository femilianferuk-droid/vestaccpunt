...
DEFAULT_PRICES = {
    ...
    "Великобритания": 70.0,
    "Аргентина": 45.0,
    # новые страны (добавлены 19 стран для расширения географии)
    "Бангладеш": 20.0,
    "Пакистан": 20.0,
    "Египет": 25.0,
    "Нигерия": 20.0,
...
COUNTRY_CODES = {
    "1": "США",          # +1 — США (Канада детектится отдельно, чтобы не конфликтовать)
    "7": "Россия",
    "91": "Индия",
    "49": "Германия",
    "55": "Бразилия",
...
# Полный список стран (36 шт.). Порядок определяет порядок в клавиатуре покупки.
COUNTRY_NAMES = [
...
# Сколько стран показывать на одной странице в клавиатуре покупки
...
def main_menu_keyboard() -> InlineKeyboardMarkup:
    """Главное меню бота"""
    builder = InlineKeyboardBuilder()
    # Кнопка-вход в Mini App (Flask-приложение app.py).
    # Если MINIAPP_URL не настроен — кнопку просто не показываем,
    # чтобы Telegram не ругался на некорректный URL.
    if MINIAPP_URL and MINIAPP_URL.startswith("https://"):
        builder.row(
            InlineKeyboardButton(
                text="🛍 Открыть маркет (Mini App)",
...
async def cmd_start(message: Message, command: CommandObject):
    ...
    # Deeplink из мини-аппа: ?start=deposit или ?start=deposit_<method>.
    # Если пришли по такой ссылке — сразу открываем меню пополнения,
    # минуя приветствие.
    start_arg = (command.args or '').strip()
    if start_arg == 'deposit' or start_arg.startswith('deposit_'):
        ...
        await send_media_message(message, "deposit", text, deposit_keyboard())
        return
...
def buy_country_keyboard(prices: dict, page: int = 0) -> InlineKeyboardMarkup:
    """Клавиатура выбора страны для покупки с пагинацией.

    На одной странице — не больше COUNTRIES_PER_PAGE стран + кнопки
    «Вперёд» / «Назад» внизу. Плюс отдельная кнопка «Все страны» сверху.
    """
    builder = InlineKeyboardBuilder()

    total = len(COUNTRY_NAMES)
    per_page = max(1, COUNTRIES_PER_PAGE)
    total_pages = max(1, (total + per_page - 1) // per_page)
    page = max(0, min(page, total_pages - 1))

    # Все страны — всегда отдельной кнопкой сверху
    builder.row(
        create_button(
            f"🌍 Все страны",
    ...
    for country in COUNTRY_NAMES[start:end]:
        flag = COUNTRY_FLAGS.get(country, "")
        builder.row(
            create_button(
                f"{flag} {country}",
    ...
    # Навигация: «Назад» / «Стр. X/Y» / «Вперёд»
    nav_row = []
    if page > 0:
        nav_row.append(
            create_button(
                "⏮ Назад",
                callback_data=f"buy_country_page_{page - 1}",
                style="primary",
                icon="back",
            )
        )
    if total_pages > 1:
        nav_row.append(
            create_button(
                ...
                callback_data="buy_country_noop",
                style="default",
                icon="info",
            )
        )
...
@router.callback_query(F.data == "buy_country_noop")
async def cb_buy_country_noop(callback: CallbackQuery):
    """Пустой обработчик для индикатора страницы (чтобы не было ошибки
    про unknown callback)."""
    await callback.answer()


@router.callback_query(F.data.startswith("buy_country_page_"))
async def cb_buy_country_page(callback: CallbackQuery):
    """Перелистывание страниц в клавиатуре выбора страны."""
    await callback.answer()
    if not await require_subscription(callback):
        return
    try:
        page = int(callback.data.replace("buy_country_page_", "", 1))
    except ValueError:
        page = 0
    prices = await get_all_prices()
    await send_media_message(
        ...
        buy_country_keyboard(prices, page=page),
    )
...
async def show_listings_page(callback: CallbackQuery, page: int = 0, country: Optional[str] = None):
    ...
    ⚠️ FIX «уже купили» (bug #1, сторона бота):
    Если аккаунт уже продан (Account.is_sold=True) — соответствующее
    объявление НЕ должно торчать в маркетплейсе, даже если его статус
    почему-то остался "active" (гонка с мини-апом, восстановление БД и т.п.).
    Раньше фильтр был только по Listing.status, из-за чего юзер видел
    объявление, кликал → «объявление больше не доступно» (или, хуже,
    доходил до cb_listing_buy и получал «аккаунт уже продан»). Теперь
    фильтруем ещё и по связанному Account.is_sold.
    """
    offset = page * LISTINGS_PAGE_SIZE

    async with async_session() as session:
        # Считаем общее число активных объявлений, чей аккаунт ещё не продан
        count_q = (
            select(func.count(Listing.id))
            .join(Account, Account.id == Listing.account_id)
            .where(Listing.status == "active", Account.is_sold == False)
        )
        if country:
...
async def cb_listing_view(callback: CallbackQuery):
    ...
    # ⚠️ FIX «уже купили» (bug #1, сторона бота):
    # Доп. проверка: даже если Listing.status='active', связанный Account
    # может быть уже продан (например, через мини-апп буквально секунду
    # назад). Раньше в этом случае мы всё равно открывали карточку и
    # показывали кнопку «КУПИТЬ», а уже на cb_listing_buy получали
    # «аккаунт уже продан» — и это выглядело как баг для пользователя.
    # Теперь сразу гасим такое объявление и говорим честно.
    async with async_session() as session:
        acc_result = await session.execute(
            select(Account).where(Account.id == listing.account_id)
        )
        linked_account = acc_result.scalar_one_or_none()
        if linked_account and linked_account.is_sold:
            # Гасим объявление, чтобы оно не висело в списке.
            listing.status = "cancelled"
            await session.commit()
            await callback.message.answer(
                f'{emoji("cross")} <b>Этот аккаунт уже куплен</b>\n\n'
                f'{emoji("info")} Объявление снято с публикации, так как '
                f'аккаунт уже был продан.',
                reply_markup=main_menu_keyboard()
            )
            return
...
# ⚠️ ВАЖНО: эти хэндлеры зарегистрированы ВЫШЕ общего my_listing_*,
# потому что в aiogram 3 dispatcher матчит хэндлеры по порядку регистрации.
# Иначе cb_my_listing_manage ловит и my_listing_cancel_5 — пытается сделать
# int("cancel_5") и валится с ValueError. Поэтому:
#   1) my_listing_cancel_*  — точное начало, идёт первым
#   2) listing_manage_*     — алиас для владельца из публичной карточки лота
#   3) my_listing_*         — общий (уже строго цифровой id после префикса)
...
async def cb_my_listing_cancel(callback: CallbackQuery):
    ...
    await callback.answer()
    try:
        listing_id = int(callback.data.replace("my_listing_cancel_", "", 1))
    except ValueError:
    ...
    async with async_session() as session:
        ...
        if account:
            # На всякий случай снимаем флаг "продан" — раньше тут просто
            # читался объект без изменений, из-за чего после отмены аккаунт
            # мог остаться помечен как проданный.
            account.is_sold = False
        await session.commit()
...
@router.callback_query(F.data.startswith("listing_manage_"))
async def cb_listing_manage(callback: CallbackQuery):
    """Алиас: открыть свою карточку лота из публичного маркетплейса.

    Кнопка 'Моё объявление' в listing_detail_keyboard использует
    callback_data='listing_manage_{id}'. Раньше обработчика не было вовсе —
    клик по кнопке просто молча игнорировался.
    """
    await callback.answer()
    try:
        listing_id = int(callback.data.replace("listing_manage_", "", 1))
    except ValueError:
        await callback.answer("Некорректный идентификатор", show_alert=True)
        return
    ...
    # Показываем ту же карточку, что и в "Мои продажи"
...
async def cb_my_listing_manage(callback: CallbackQuery):
    """Карточка моего объявления (только для callback вида my_listing_<int>).

    Регистрируется ПОСЛЕ cancel/manage, поэтому 'cancel_5' и 'manage_5'
    сюда уже не доходят. Дополнительно страхуемся — если после
    'my_listing_' идёт не число, молча выходим (защита от любых будущих
    под-prefix'ов).
    """
    await callback.answer()
    raw = callback.data.replace("my_listing_", "", 1)
    if not raw.isdigit():
        # Сюда попадут только настоящие my_listing_<int>; всё остальное уже
        # перехвачено более специфичными хэндлерами выше.
        return
    listing_id = int(raw)
